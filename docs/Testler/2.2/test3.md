...
|---|---|---|---|---|---|---|
| DeepSeek V4 Pro | 74 | 86 | 79 | 89 | 34 | 84 |
| Qwen3.6 Plus | 75 | 88 | 73 | 93 | 38 | 84 |
| Gemini 3.5 Flash | 82 | 90 | 75 | 95 | 52 | 99 |
...
|---|---|---|---|---|---|
| Okunabilirlik | 93 | 79 | — | **86** | Ortalama (J1-J2) |
| Performans | 85 | 73 | — | **79** | Ortalama (J1-J2) |
...
**Öneriler:**
- Timeout süreleri için (ör. 400, 900, 1800, 2200) anlamlı sabitler tanımlayın (ör. PULSE_DURATION, SEND_DELAY) ki amaçları daha açık olsun.
- Kategori sekmeleri ve 'Send to Kitchen' butonundaki tekrarlanan class ekleme/çıkarma işlemleri için yardımcı fonksiyonlar yazarak kod tekrarını azaltın.
- Fix inconsistent indentation in the menuItems array so the `desc:` line of every object aligns with its siblings.
- Replace inline `onclick` template strings with event delegation on the grid/list containers to eliminate escaped quotes and long lines.

### Performans — 79/100

**Öneriler:**
- Kategori değişiminde veya sepete ekleme/çıkarma işlemlerinde tüm DOM'un `innerHTML` ile yeniden oluşturulması yerine, sadece değişen öğelerin güncellendiği daha hedeflenmiş bir yaklaşım benimseyin. Bu, gelecekte menü öğeleri sayısı arttığında performansı koruyacaktır.
- Her güncellemede tüm özet alanlarını (`cartSubtotal`, `cartTax`, vb.) tek tek `textContent` ile güncellemek yerine, bir template literal ile tek seferde oluşturup `innerHTML` ile atamak DOM manipülasyon sayısını azaltabilir.
- Tailwind CDN'si (~3MB JS runtime derlemesi) üretim ortamında ciddi gecikmeye yol açar; build-time Tailwind CLI veya PostCSS kullanın.
- Sepet güncellemelerinde tüm listeyi innerHTML ile yeniden çizmek yerine, sadece değişen satırı güncelleyin veya DocumentFragment ile toplu ekleme yaparak layout thrashing'i azaltın.

### Güvenlik — 89/100

**Öneriler:**
- Kullanıcı girdisi veya API'dan gelen veri ile innerHTML kullanılacaksa, XSS'e karşı DOMPurify gibi bir sanitizasyon kütüphanesi entegre edin.
- Gerçek bir uygulamada 'Send to Kitchen' butonu arka uca istek atacaksa, HTTPS kullanıldığından ve hassas verilerin console'da sızmayacağından emin olun.
- `innerHTML` yerine `textContent` veya DOM API (`createElement`) kullanarak içerik render edin; onclick handler'larına veri geçmek için `data-*` attribute'ları ve `addEventListener` tercih edin.
- Tüm kullanıcıya gösterilecek dinamik metinleri (isim, açıklama) mutlaka escape eden bir yardımcı fonksiyon ile sanitize edin; böylece gelecekte API'den gelen veriler eklendiğinde XSS riski otomatik olarak önlenmiş olur.

### Sürdürülebilirlik — 34/100

**Öneriler:**
- Bileşenleri (Header, MenuGrid, CartPanel, Toast) ayrı dosyalara/componentlere ayırın, durumu yönetmek için bir state management (veya context) kullanın ve iş mantığını custom hook'lara taşıyın.
- Sabit değerleri (TAX_RATE, SERVICE_FEE_RATE, menuItems) ve yapılandırma verilerini ayrı constants/config dosyalarına çıkarın, böylece değişiklikler tek noktadan yönetilir.
- Refactor the JavaScript into modules (e.g., data.js for menu items, cart.js for state logic, ui.js for rendering) and replace inline onclick handlers with event delegation to eliminate brittle string interpolation and improve testability.
- Centralize category/UI state and button class transitions in a small state object or helper functions instead of repeating classList.add/remove blocks, so changing button styles or adding categories only requires editing one place.
...
**Öneriler:**
- Yer tutucu metin veya TODO bulunmuyor, tüm bileşenler eksiksiz çalışır durumda.
- Tüm kısıtlar tam olarak karşılanmış.
- Prompt'ta istenmemesine rağmen ekstra 'Service Fee' satırı eklenmiş; kullanıcı talimatına sadık kalmak için yalnızca Subtotal, Tax ve Total yeterli olabilirdi.
- Sağ pane için sabit bir genişlik (örn. w-96) yerine responsive kesme noktaları daha tutarlı bir yerleşim sunabilirdi; mobilde split-screen yerleşimi bozulabilir.
...
| Performans | 65 | 80 | — | **73** | Ortalama (J1-J2) |
| Güvenlik | 95 | 90 | — | **93** | Ortalama (J1-J2) |
| Sürdürülebilirlik | 25 | 52 | 38 | **38** | Medyan (J1·J2·J3) — tahkim |
| Prompt Uyumu | 85 | 83 | — | **84** | Ortalama (J1-J2) |
...
**Öneriler:**
- Sabit değerler (ör. `0.085` vergi oranı) için anlamlı sabitler tanımlayarak kodu daha anlaşılır hale getirebilirsiniz.
- Boş sepet HTML'ini bir değişkene atayarak `renderCart` içindeki tekrarı önleyebilir ve kodu daha kuru tutabilirsiniz.
- Vergi oranı (0.085) ve zamanlama değerleri (800, 1200) için dosya başında TAX_RATE, SEND_ANIMATION_MS, SUCCESS_DISPLAY_MS gibi adlandırılmış sabitler tanımlayın.
- Boş sepet durumunun HTML/SVG'sini bir fonksiyona (ör. renderEmptyCart()) çıkarıp hem başlangıçta hem de renderCart içinde yeniden kullanarak kod tekrarını ortadan kaldırın.

### Performans — 73/100

**Öneriler:**
- `addToCart` işleminde `menuData.find` yerine sabit bir `Map` veya `object` (örn. menuById) kullanarak O(1) arama yapılmalı.
- Kategori ve menü render işlemleri innerHTML yerine DOM manipülasyonu veya virtual-DOM yaklaşımıyla diff yapılarak gereksiz render'lar önlenmeli.
- Cart değişikliklerinde `innerHTML` ile tüm DOM'u yeniden oluşturmak yerine, sadece değişen satırı güncellemek büyük sepetlerde daha verimli olur (event delegation ile).
- Menü verisi büyürse, `addToCart` içindeki `menuData.find()` yerine id-keyed bir `Map` kullanmak O(1) erişim sağlar.

### Güvenlik — 93/100

**Öneriler:**
- Linklere ve görsellere dış kaynaklardan erişiliyorsa, HTTPS zorunluluğunu sağlamak için protokolü denetleyen bir güvenlik politikası (Content Security Policy) eklemeyi değerlendirebilirsiniz.
- İleride sunucu entegrasyonu eklenirse API çağrılarından dönen hata mesajlarının doğrudan kullanıcıya yansıtılmamasına dikkat edin.
- İçerik Güvenlik Politikası (CSP) için bir <meta http-equiv='Content-Security-Policy'> etiketi ekleyin ve satır içi onclick handler'larını addEventListener ile değiştirerek 'unsafe-inline' ihtiyacını azaltın.
- innerHTML ile render edilen bölümleri element.textContent veya DOM API (createElement) ile oluşturarak, gelecekte dinamik veri kaynağı eklenmesi durumunda oluşabilecek XSS riskini baştan önleyin; ayrıca CDN'den yüklenen Tailwind/Google Fonts için Subresource Integrity (SRI) hash'leri ekleyin.

### Sürdürülebilirlik — 38/100

**Öneriler:**
- İş mantığını (state, hesaplama) UI render mantığından ayırmak için ayrı bir JavaScript modülü oluşturun; state yönetimi için basit bir store veya hook yapısı kullanın.
- Sabit değerleri (vergi oranı, kategoriler) bir config nesnesine çıkarın; API veya veri işlemleri için veri katmanı soyutlayın.
- Vergi oranı, masa numarası, kategori listesi gibi değerleri `const CONFIG = { TAX_RATE: 0.085, TABLE: '12' }` şeklinde ayrı bir config bölümüne çıkarın; boş sepet HTML'ini tek bir `renderEmptyState()` fonksiyonuna taşıyıp tekrarı önleyin.
- State'i (cart, activeCategory) tek bir merkezi `state` objesine alıp render fonksiyonlarını saf (pure) hale getirin; 'Send to Kitchen' başarı/hata akışı için loading/error/success durumlarını net olarak yönetin ve mümkünse fonksiyonları dışarıdan bağımlılık alacak şekilde (parametre enjeksiyonu) tasarlayarak test edilebilirliği artırın.
...
**Öneriler:**
- Menü öğeleri kartlarına daha belirgin bir ürün resmi veya ikonu eklenerek görsel uyum artırılabilir (placeholder yerine).
- Sepet boşken veya sipariş gönderilirken kullanıcıya daha net geri bildirim animasyonları eklenebilir.
- Gerçek ürün görselleri için placeholder image service (ör. Unsplash) entegrasyonu eklenebilir, menü öğeleri için görsel tutarlılık sağlanır.
- Her menü kartında 'Add +' butonu yerine doğrudan tüm karta tıklanabilir alan (clickable card) yapılabilir, mobil kullanım kolaylığı artırılır.
...
| Sürdürülebilirlik | 35 | 58 | 52 | **52** | Medyan (J1·J2·J3) — tahkim |
...
**Öneriler:**
- Vergi oranı (0.08) ve servis ücreti (0.05) değerlerini `VAT_RATE` ve `SERVICE_CHARGE_RATE` gibi anlamlı sabitlerle değiştirin.
- Fiyat hesaplamalarında kullanılan sayılar sabit olarak tanımlanırsa ileride değişiklik yapmak daha kolay olur.
- Vergi ve servis oranları (0.08, 0.05) ile sabit değerler 'TAX_RATE' ve 'SERVICE_CHARGE_RATE' gibi isimlendirilmiş sabitlere çıkarılmalı.
- Bölüm başlıkları yorumlarla (// LEFT PANE, // RIGHT PANE) ayrılmış; bunlar semantik HTML etiketleri (main, aside) veya daha açıklayıcı niteliklerle desteklenebilir.

### Performans — 75/100

**Öneriler:**
- Arama sorgusuna `debounce` uygulayarak her tuş vuruşunda `filteredItems()` fonksiyonunun gereksiz yere çalışmasını engelleyin.
- `getSubtotal()` gibi hesaplamalar her reaktif güncellemede yeniden çalışıyor. Bu değerleri bir `get` computed property veya `x-effect` ile manuel olarak cache'leyerek gereksiz tekrarlı hesaplamaları azaltabilirsiniz.
- `Intl.NumberFormat` örneğini fonksiyon dışına (modül kapsamına) taşıyarak `formatPrice` içinde tekrar tekrar oluşturulmasını önleyin; bu yüzlerce çağrıda ciddi kazanç sağlar.
- `filteredItems()` ve `getSubtotal()` gibi türetilmiş değerleri Alpine.js'te `get` accessor'ları veya `Alpine.effect` ile bir kez hesaplayıp cache'leyin; arama inputuna debounce ekleyin.

### Güvenlik — 95/100

**Öneriler:**
- Harici API entegrasyonunda HTTPS zorunlu tutulmalı (fetch çağrılarında URL kontrolü veya sabit HTTPS şeması kullanılabilir).
- Sipariş gönderimi sırasında hata mesajları kullanıcıya direkt yansıtılırsa backend'ten gelen hata detayları sanitize edilmelidir; şu anda sahte gecikme kullanılıyor ancak gerçek API'ye bağlanıldığında hata sızıntısı engellenmelidir.
- Tailwind CSS ve Alpine.js CDN'lerine `integrity` ve `crossorigin` öznitelikleri ekleyerek Subresource Integrity (SRI) koruması sağlayın.
- `<meta http-equiv="Content-Security-Policy" content="default-src 'self'; script-src 'self' https://cdn.tailwindcss.com https://cdn.jsdelivr.net; img-src 'self' https://images.unsplash.com data:; style-src 'self' 'unsafe-inline' https://fonts.googleapis.com; font-src https://fonts.gstatic.com;">` şeklinde bir CSP meta etiketi ekleyerek inline script ve yetkisiz kaynakları kısıtlayın.

### Sürdürülebilirlik — 52/100

**Öneriler:**
- Menü verisini, kategorileri ve sabit oranları (vergi, servis ücreti) ayrı bir constants.js dosyasına taşıyın.
- Cart ve ürün filtreleme mantığını reusable bir custom hook (ör. useCart, useMenuFilter) içine alarak mantık ve sunum katmanını birbirinden ayırın; API çağrıları için error/loading durumlarını da yönetin.
- Sabit değerleri (vergi oranı %8, servis bedeli %5, para birimi) ayrı bir config/constants dosyasına çıkararak magic number'ları ortadan kaldırın; menü verisini JSON veya ayrı bir data modülüne taşıyarak sunum katmanından ayırın.
- sendToKitchen fonksiyonuna try/catch ile hata state'i (isError, errorMessage) ekleyin; başarısız API yanıtı durumunda kullanıcıya anlamlı bir hata mesajı ve tekrar deneme (retry) imkânı sunun.
...
**Öneriler:**
- Kategoriler altında 'All' seçeneği dışındaki kategorilerle eşleşen menü öğeleri sınırlı olsa da prompt bunu zorunlu kılmıyor; ancak daha zengin bir menü verisi ile tam işlevsellik artırılabilir.
- Vergi hesaplaması sabit oranlarla yapılmış, prompt'ta spesifik vergi detayı istenmese de farklı vergi oranları için yapılandırılabilir hale getirmek ileride faydalı olur.
- Senaryoda belirtilmemesine rağmen, cart'ta başlangıçta dummy veri yüklenmesi (init) kullanıcıyı yanıltabilir; boş durumun (empty state) görünmesi için bu kaldırılabilir.
- Mobil görünüm için responsive davranış (ör. sağ panelin drawer/tab'a dönüşmesi) prompt'ta istenmese de UX açısından eklenebilir.
