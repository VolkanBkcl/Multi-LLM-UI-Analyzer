...
|---|---|---|---|---|---|---|
| DeepSeek V4 Pro | 71 | 47 | 91 | 98 | 25 | 96 |
| Qwen3.6 Plus | 77 | 86 | 86 | 98 | 40 | 73 |
| Gemini 3.5 Flash | 80 | 78 | 91 | 98 | 57 | 77 |
...
|---|---|---|---|---|---|
| Okunabilirlik | 52 | 42 | — | **47** | Ortalama (J1-J2) |
| Performans | 100 | 82 | — | **91** | Ortalama (J1-J2) |
| Güvenlik | 100 | 95 | — | **98** | Ortalama (J1-J2) |
| Sürdürülebilirlik | 20 | 30 | — | **25** | Ortalama (J1-J2) |
| Prompt Uyumu | 98 | 94 | — | **96** | Ortalama (J1-J2) |
...
### Okunabilirlik — 47/100

**Öneriler:**
- Tekrarlayan modül bloklarını JavaScript'te bir veri yapısından dinamik olarak render edip tek satırlık buton stillerini CSS sınıflarına taşıyın.
- Magic number olarak kullanılan renk kodlarını (ör: 'rgba(99,102,241,0.12)') ve piksel değerlerini anlamlı CSS değişkenleri veya sabitlerle değiştirin.
- Ortak stilleri (modül başlığı, ders satırı, hover durumu gibi) birkaç CSS sınıfına çıkarıp class="..." kullanmak, kopya-yapıştır tekrarını ve satır uzunluğunu büyük ölçüde azaltır.
- Satır başına düşen stilleri birden fazla satıra bölerek veya stil değerlerini kök CSS değişkenlerine (--primary, --bg-elevated vb.) taşıyarak okunabilirliği artırıp magic number tekrarını önleyin.

### Performans — 91/100

**Öneriler:**
- Accordion listesi çok sayıda modül içerirse, tüm akordiyonları taramak yerine aktif olanı bir değişkende tutarak performans iyileştirilebilir.
- Animasyonlu geçişlerde `max-height` yerine `height` veya CSS transform kullanımı daha düşük yeniden boyama maliyeti sağlayabilir, ancak mevcut durumda ihmal edilebilir düzeyde.
- toggleAccordion içinde document.querySelectorAll('.acc-content') her tıklamada yeniden çalıştırılıyor; modül butonlarına ortak bir delegated click listener ekleyip yalnızca tıklanan öğenin durumunu değiştirerek gereksiz DOM sorgusunu ortadan kaldırabilirsin.
- scrollHeight yerine CSS custom property + grid-template-rows: 0fr/1fr tekniği (veya max-height: fit-content) kullanırsan, içerik yüksekliği değiştiğinde layout thrashing azalır ve animasyon daha akıcı olur.

### Güvenlik — 98/100

**Öneriler:**
- Dinamik içerik eklenmesi durumunda kullanılacak sanitizasyon için DOMPurify entegrasyonu önerilir.
- Sunucu tarafında Content-Security-Policy başlığı kullanılarak olası XSS saldırılarına karşı ek koruma sağlanabilir.
- Gelecekte dinamik içerik (ör. API'den gelen ders başlıkları) eklenirse, kullanıcı girdisini textContent ile render edin veya DOMPurify ile sanitize edin.
- Gerekli olduğunda API çağrıları için HTTPS zorunluluğu ve hassas token'ları HttpOnly cookie'de saklama gibi kontrolleri ekleyin.

### Sürdürülebilirlik — 25/100

**Öneriler:**
- HTML yapısını, CSS stillerini ve JavaScript mantığını ayrı dosyalara veya en azından ayrı <style> ve <script> bloklarına taşıyın; inline style kullanımını bırakıp CSS sınıfları ile çalışın.
- Akordiyon modüllerini tekrar kullanılabilir bir bileşen haline getirin (ör. bir template veya JavaScript fonksiyonu ile oluşturarak) böylece her modül için aynı HTML bloğunu kopyalamak zorunda kalmazsınız.
- Curriculum modül ve derslerini bir JavaScript dizisinden render ederek ~200 satırlık tekrarı ortadan kaldırın; bu sayede yeni modül eklemek tek satır veri eklemek kadar kolaylaşır.
- Renkler, boşluklar ve tipografi için <style> bloğunda CSS custom properties (--color-accent, --radius-card vb.) tanımlayın; böylece tema değişiklikleri tek noktadan yapılabilir.
...
**Öneriler:**
- Duyarlı (responsive) davranış için CSS media queries veya flex-wrap ayarlarıyla daha küçük ekranlarda sidebar'ın daha uygun şekilde konumlanması sağlanabilir.
- Accordion butonlarındaki inline onclick attribute'ları yerine addEventListener kullanımı, kodun sürdürülebilirliğini ve modern standartlara uyumunu artıracaktır.
- Mobil/masaüstü geçişleri için en az bir media query ekleyerek 768px altında layout'u dikey istifeye çevirin (gerçek responsive davranış).
- Scrollbar ve pulse animasyonu için <style> blokları yerine bu kuralları inline style attribute'larına veya tek bir <style> bloğuna taşıyarak 'inline CSS' kısıtına tam uyum sağlayın.
...
| Performans | 98 | 76 | 86 | **86** | Medyan (J1·J2·J3) — tahkim |
| Güvenlik | 100 | 95 | — | **98** | Ortalama (J1-J2) |
| Sürdürülebilirlik | 40 | 40 | — | **40** | Ortalama (J1-J2) |
| Prompt Uyumu | 74 | 72 | — | **73** | Ortalama (J1-J2) |
...
**Öneriler:**
- Renkler ve tekrar eden değerler için CSS özel değişkenleri (custom properties) kullanarak kodun okunabilirliğini ve sürdürülebilirliğini artırabilirsiniz.
- 'play-btn' gibi sınıf adlarında kısaltma yerine tam kelimeler (ör. 'play-button') tercih ederek evrensel olmayan kısaltmalardan kaçının.
- Tekrarlanan play/check ikon SVG'lerini bir <symbol> + <use> sprite yapısıyla birleştirerek DRY ihlalini giderin ve bakımı kolaylaştırın.
- Sık kullanılan renkler, boşluk değerleri ve animasyon süreleri için CSS custom properties (--color-primary, --radius-sm, --transition-base) tanımlayarak magic number'ları merkezileştirin.

### Performans — 86/100

**Öneriler:**
- Inline SVG'ler <defs> ve <use> ile referans verilerek DOM boyutu azaltılabilir.
- Hover efektlerindeki transformlar için will-change: transform eklenerek GPU katmanı önceden hazırlanabilir.
- `transition: all` yerine yalnızca değişen özellikleri (ör. `background`, `transform`) listeleyerek gereksiz reflow/repaint'leri önleyin.
- Akordeon için `max-height: 1000px` geçişi yerine `grid-template-rows: 0fr/1fr` tekniğini kullanarak layout hesaplamasını daha verimli hale getirin.

### Güvenlik — 98/100

**Öneriler:**
- Eksik olan `toggleModule` fonksiyonu ileride DOM manipülasyonu içeriyorsa, `innerHTML` gibi güvensiz yöntemlerden kaçınılmalıdır.
- Inline `onclick` kullanımı yerine olay dinleyicileri (event listener) eklemek, kodun bakımını kolaylaştırır ve olası güvenlik risklerini azaltır.
- Mümkünse inline onclick gibi inline event handler'lar yerine addEventListener kullanarak CSP (Content Security Policy) uyumluluğunu artırın.
- Gelecekte dinamik içerik veya API entegrasyonu eklenecekse, dış kaynakları yalnızca HTTPS üzerinden yükleyin ve kullanıcı girdilerini render etmeden önce sanitize edin.

### Sürdürülebilirlik — 40/100

**Öneriler:**
- Renkleri ve stil token'larını CSS custom property'leri olarak tanımlayarak değiştirilebilirliği artırın.
- Modüller ve dersler gibi verileri bir JavaScript dizisinden dinamik olarak oluşturacak şekilde yeniden düzenleyerek kodu daha modüler ve sürdürülebilir hale getirin.
- Modül ve dersleri bir JS dizisinden (curriculum data) render eden küçük bir fonksiyon ekleyerek HTML tekrarını ortadan kaldırın; böylece veri kaynaklı değişiklikler tek noktadan yönetilir.
- Renk paleti, spacing ve font boyutları için CSS custom properties (örn. --color-primary, --space-md) tanımlayın; tekrarlayan renkleri ve değerleri token olarak merkezileştirerek modifiability'yi artırın.
...
**Öneriler:**
- Accordion'un düzgün çalışması için onclick ile çağrılan toggleModule fonksiyonunun JavaScript ile tanımlanması gerekir.
- Alternatif olarak, JavaScript kullanmadan accordion efekti için <details> ve <summary> etiketlerinden yararlanılabilir.
- Modül açma/kapama için başvurulan toggleModule fonksiyonunun tanımını eksiksiz ekleyin; bu olmadan accordion etkileşimi çalışmaz.
- Modül 3'te belirtilen ders sayısı (6) ile görüntülenen ders sayısı (4) arasındaki tutarsızlığı giderin; tüm modülleri eksiksiz tamamlayın.
...
| Metrik | J1 | J2 | J3 | Final | Çözüm Yöntemi |
|---|---|---|---|---|---|
| Okunabilirlik | 90 | 65 | 78 | **78** | Medyan (J1·J2·J3) — tahkim |
| Performans | 100 | 82 | — | **91** | Ortalama (J1-J2) |
| Güvenlik | 100 | 95 | — | **98** | Ortalama (J1-J2) |
| Sürdürülebilirlik | 55 | 58 | — | **57** | Ortalama (J1-J2) |
...
**Öneriler:**
- Geçersiz CSS özelliği `group: hover;` kaldırılmalı veya düzeltilmeli; bu okuyucuyu yanıltabilir.
- `onclick="toggleModule(this)"` ile çağrılan işlev tanımlanmadığı için, kodun bu kısmı eksik görünüyor; işlev eklenmeli veya yorumla açıklanmalı.
- `.video-placeholder` içindeki geçersiz `group: hover;` satırını kaldırın — bu Tailwind'e özgü bir yardımcı sınıftır ve standart CSS'te işlevsizdir.
- İlerleme yüzdesi gibi değerler için CSS değişkeni (örn. `--progress: 42%;`) tanımlayıp `width: var(--progress)` kullanarak sihirli sayıları anlamlandırın.

### Performans — 91/100

**Öneriler:**
- Geçersiz CSS kuralı `group: hover;` kaldırılmalıdır.
- Accordion animasyonu için `will-change: grid-template-rows` eklenerek animasyon performansı iyileştirilebilir (isteğe bağlı).
- Tanımsız toggleModule fonksiyonunu ekleyin veya onclick çağrılarını kaldırın; aksi halde her tıklamada konsol hatası oluşur.
- Geçersiz 'group: hover;' kuralını kaldırın; bu hiçbir şey yapmaz ve tarayıcı tarafından yok sayılır.

### Güvenlik — 98/100

**Öneriler:**
- Eğer bu yapı daha sonra dinamik veri (örn. video başlıkları, ders açıklamaları) ile beslenecekse, kullanıcı tarafından oluşturulan içerikler JSX'e eklenmeden önce XSS'e karşı temizlenmelidir.
- Canlı bir uygulamada, curriculum listesindeki ders verileri bir API'den çekilecekse, gelen veriler render edilmeden önce içerik doğrulaması yapılmalıdır.
- toggleModule fonksiyonunu eklerken innerHTML/eval kullanmaktan kaçınarak yalnızca classList.toggle gibi güvenli DOM API'leri tercih edin.
- Harici kaynak (font, video, CDN) eklenecekse 'https://' protokolünü zorunlu kılın ve integrity (SRI) özniteliği kullanın.

### Sürdürülebilirlik — 57/100

**Öneriler:**
- Modül ve ders verilerini bir JavaScript dizisi/objesi olarak ayrı bir veri dosyasına (ör. `curriculumData.js`) taşıyın ve bu veriyi map fonksiyonu ile renderlayın; bu tekrarı önler ve güncellemeyi kolaylaştırır.
- Accordion toggle mantığını `onclick="toggleModule(this)"` yerine bir JavaScript event listener veya basit bir bileşen mantığı ile yöneterek HTML'i temiz tutun ve sunum/iş mantığını ayırın.
- Tekrar eden SVG ikonlarını <svg><symbol id='...'> + <use href='#...'> yapısıyla soyutlayarak hem dosya boyutunu hem bakım maliyetini düşürün.
- Müfredat verisini bir JavaScript dizisine taşıyıp render edin; böylece yeni modül/ekleme tek bir veri kaynağından yönetilebilir ve toggleModule gibi state mantığı da merkezi olur.
...
**Öneriler:**
- Video placeholder yerine gerçek bir video oynatıcı (HTML5 video etiketi) entegre edilebilir.
- Sidebar akordeon işlevselliği için JavaScript toggle fonksiyonu eksik, onclick attribute'una rağmen. Bu eklenmeli.
- Complete the truncated markup so all lessons/closing tags are properly closed and the document is valid HTML.
- Ensure the toggleModule JavaScript function referenced in onclick handlers is defined so the accordion behavior is fully functional.
