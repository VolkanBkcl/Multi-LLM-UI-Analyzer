...
|---|---|---|---|---|---|---|
| DeepSeek V4 Pro | 68 | 36 | 80 | 96 | 30 | 96 |
| Qwen3.6 Plus | 78 | 90 | 93 | 95 | 33 | 77 |
| Gemini 3.5 Flash | 75 | 69 | 93 | 90 | 49 | 75 |
...
|---|---|---|---|---|---|
| Okunabilirlik | 34 | 38 | — | **36** | Ortalama (J1-J2) |
| Performans | 94 | 72 | 80 | **80** | Medyan (J1·J2·J3) — tahkim |
| Güvenlik | 100 | 92 | — | **96** | Ortalama (J1-J2) |
| Sürdürülebilirlik | 20 | 40 | — | **30** | Ortalama (J1-J2) |
| Prompt Uyumu | 98 | 93 | — | **96** | Ortalama (J1-J2) |
...
### Okunabilirlik — 36/100

**Öneriler:**
- Tekrar eden modül ve ders yapılarını HTML içine manuel kopyalamak yerine, JavaScript ile bir dizi üzerinden dinamik olarak render ederek kod tekrarını (DRY ihlali) ortadan kaldırın.
- Satır içi `onmouseover`/`onmouseout` olay yönetimini kaldırıp, stilleri CSS sınıfları (`:hover` vb.) ile yönetin; `toggleAccordion` fonksiyonundaki `active-acc` sınıf kontrolünü daha açıklayıcı bir boolean değişkenle (`isOpen`) netleştirin.
- Beş modülün ortak yapısını (button + acc-content) tek bir JavaScript template ile üret veya class tabanlı stil kullanarak tekrarı azalt; bu, dosya uzunluğunu yarıya indirecektir.
- Satır uzunluğunu 120 karakterin altında tutmak için inline style'ları <style> bloğunda semantik class'lara (ör. .module-header, .lesson-item) taşı.

### Performans — 80/100

**Öneriler:**
- Mevcut akordeon işlevinde `document.querySelectorAll('.acc-content')` sorgusunu, üst kapsayıcı üzerinde `container.querySelectorAll` ile sınırlayarak DOM arama kapsamını daraltabilirsiniz. Büyük ölçekte bir kazanç olmasa da en iyi pratikler için faydalıdır.
- Inline `onclick` ve `onmouseover`/`onmouseout` nitelikleri yerine tek bir olay dinleyici (event delegation) kullanmak, özellikle ders listesi büyüdüğünde bellek ayak izini azaltır.
- `toggleAccordion` içinde `.acc-content` öğelerini sayfa yüklenirken bir kez seçip önbelleğe alarak her tıklamada DOM taramasını önleyin.
- Sürekli çalışan `pulse-dot` animasyonu ve geçişlere `@media (prefers-reduced-motion: reduce)` desteği ekleyerek düşük güçlü cihazlarda gereksiz paint/composite maliyetini azaltın.

### Güvenlik — 96/100

**Öneriler:**
- Eğer bu yapı ileride dinamik içerikle (örn. kullanıcı yorumu veya dış API'den gelen video başlığı) birleştirilecekse, içeriği JSX'e veya DOM'a eklemeden önce mutlaka DOMPurify gibi bir kütüphane ile sanitize edin.
- Harici script veya kaynak (CDN, analytics) eklenecekse Subresource Integrity (SRI) kullanarak ve `Content-Security-Policy` header'ı ile güvenliği artırın.
- İnline event handler'lar (onclick, onmouseover/onmouseout) yerine addEventListener kullanarak daha temiz ve CSP-uyumlu bir yapıya geçin.
- HTML meta etiketi ile bir Content Security Policy tanımlayarak inline script/style kullanımını kısıtlayın (script-src 'self' gibi).

### Sürdürülebilirlik — 30/100

**Öneriler:**
- Tekrar eden akordiyon HTML yapısını bir döngü ve veri kaynağından render edecek şekilde yeniden düzenleyin; manuel olarak yazılmış 5 modülü dinamik hale getirin.
- Inline CSS'i ve dağınık <style> bloklarını kaldırın; stilleri bir CSS sınıf yapısına veya dahili bir stylesheet'e taşıyarak sunum ve yapıyı birbirinden ayırın.
- Modül ve ders öğelerini JavaScript ile bir veri dizisinden render eden fonksiyonlara (renderModule, renderLesson) soyutlayıp kopyala-yapıştır tekrarını ortadan kaldırın; renkleri ve boyutları CSS değişkenleri olarak :root üzerinden tanımlayın.
- toggleAccordion fonksiyonuna null guard'ları ekleyin, ARIA öznitelikleri (aria-expanded, role='button') ve klavye desteği (Enter/Space) ekleyerek hem sürdürülebilirliği hem erişilebilirliği artırın.

### Prompt Uyumu — 96/100

- Programatik: 100/100 · Semantik: 96/100
- İhlaller: yok

**Öneriler:**
- The '24 lessons' badge and total listed lessons may be slightly inconsistent; ensure displayed counts exactly match the actual number of lessons across modules to avoid a minor semantic mismatch.
- Consider adding more explicit responsive behavior (e.g., media-query-like adjustments in inline styles) if the prompt "responsive" requirement implies a mobile/stacked layout, though the current flex layout is adequate for most screens.
- Mobil ve tablet görünümleri için @media query ekleyerek sidebar'ın altta stack olmasını veya daralmasını sağlayın (gerçek responsive davranış).
- Keyframe animasyonu ve ::-webkit-scrollbar stillerini tamamen inline yapıya taşıyın veya en azından <style> bloğunu minimumda tutun; kullanıcı 'inline CSS' istemişti.
...
| Metrik | J1 | J2 | J3 | Final | Çözüm Yöntemi |
|---|---|---|---|---|---|
| Okunabilirlik | 98 | 82 | — | **90** | Ortalama (J1-J2) |
| Performans | 98 | 88 | — | **93** | Ortalama (J1-J2) |
| Güvenlik | 95 | 95 | — | **95** | Ortalama (J1-J2) |
| Sürdürülebilirlik | 25 | 40 | — | **33** | Ortalama (J1-J2) |
| Prompt Uyumu | 78 | 75 | — | **77** | Ortalama (J1-J2) |
...
**Öneriler:**
- Renkler için CSS özel değişkenleri (custom properties) kullanarak kodun bakımını ve okunabilirliğini daha da artırabilirsiniz.
- Uzun SVG ikonlarını harici dosyalara taşıyarak HTML bölümünü sadeleştirebilirsiniz.
- Tekrar eden renkler ve transition süreleri için CSS custom properties (`:root` değişkenleri) tanımlayarak DRY ihlalini azaltın ve bakımı kolaylaştırın.
- `toggleModule` fonksiyonunu script bloğu içinde tanımlayın veya modülleri başlangıçta HTML'de `hidden` özniteliği ile işaretleyerek JS bağımlılığını netleştirin.

### Performans — 93/100

**Öneriler:**
- Büyük SVG path verileri inline olarak kullanılmış. Tekrar eden ikonlar için SVG sprite veya CSS mask tekniği kullanılarak bellek ve ağ yükü azaltılabilir.
- Accordion işlevselliği inline onclick ile sağlanmış, büyük bir listeye dönüşürse olay delegasyonu kullanmak daha verimli olabilir.
- Akordeon açılışı için max-height: 1000px geçişi yerine grid-template-rows: 0fr -> 1fr tekniği kullanın; bu, yalnızca içerik yüksekliği kadar reflow yaratır ve daha performanslıdır.
- Çok sayıda modül/lesson olması durumunda curriculum container'a content-visibility: auto ekleyerek off-screen render maliyetini azaltabilirsiniz.

### Güvenlik — 95/100

**Öneriler:**
- Üretim ortamında benzer bir yapı React gibi bir framework ile geliştirilirse, `onclick` yerine framework event binding kullanılmalı ve JSX escapelerinin doğal XSS korumasından yararlanılmalıdır.
- Eğer ders içerikleri dinamik olarak yüklenecekse, kullanıcı/API kaynaklı veriler render edilmeden önce DOMPurify gibi bir kütüphane ile sanitize edilmelidir.
- Gelecekte dinamik içerik veya kullanıcı verisi eklenecekse, tüm inputlar için HTML escape ve DOMPurify gibi sanitization uygulayın.
- Inline onclick handler yerine addEventListener kullanmak hem CSP uyumluluğu hem de bakım açısından daha güvenli bir pratiktir.

### Sürdürülebilirlik — 33/100

**Öneriler:**
- Tekrar eden ders ve modül bloklarını bir JavaScript dizisinden render etmek için bir template yapısı veya küçük bileşenler oluşturun.
- SVG ikonlar ve renk gibi stil sabitleri ayrı değişkenlerde veya CSS custom property'lerinde tanımlanarak tek bir noktadan değiştirilebilir hale getirilsin.
- Define CSS custom properties (e.g. --color-primary, --color-border) for all repeated colors and spacing tokens to centralize the design system.
- Extract curriculum data into a JavaScript array and render modules/lessons via a render function, and define the missing `toggleModule` function to make the accordion actually work and the code data-driven.
...
**Öneriler:**
- Video oynatıcı sol tarafta olsa da, sidebar'ın sağda olması için HTML yapısı korunmalı; mobilde üst üste gelmesi responsive gereksinimi karşılıyor ancak sidebar'ın sağda olduğundan emin olunmalı.
- Accordion işlevselliği JavaScript ile sağlanmış, ancak daha erişilebilir olması için ARIA roller ve klavye desteği eklenebilir.
- JavaScript ile `toggleModule` fonksiyonunun tanımını ekleyin (kodda onclick ile çağrılıyor ancak fonksiyon gövdesi eksik), böylece akordeon etkileşimi gerçekten çalışır.
- Sidebar'ın mobil görünümde daha iyi kullanıcı deneyimi için yükseklik ve kaydırma davranışını iyileştirin (örn. 50vh yerine içeriğe göre dinamik yükseklik).
...
| Metrik | J1 | J2 | J3 | Final | Çözüm Yöntemi |
|---|---|---|---|---|---|
| Okunabilirlik | 65 | 72 | — | **69** | Ortalama (J1-J2) |
| Performans | 100 | 85 | — | **93** | Ortalama (J1-J2) |
| Güvenlik | 95 | 85 | — | **90** | Ortalama (J1-J2) |
| Sürdürülebilirlik | 45 | 52 | — | **49** | Ortalama (J1-J2) |
| Prompt Uyumu | 80 | 69 | — | **75** | Ortalama (J1-J2) |
...
**Öneriler:**
- CSS'deki geçersiz `group: hover;` kuralını kaldırın veya düzeltin, aksi takdirde okuyucu için kafa karıştırıcıdır.
- Akordiyon işlevselliği için `toggleModule` fonksiyonunun tanımını ekleyin; aksi halde eksik kod, okunabilirliği ve anlaşılabilirliği zedeler.
- Tekrarlanan SVG ikonlarını (saat, onay işareti) <defs>/<symbol> ile tanımlayıp <use> ile çağırarak DRY ihlalini giderin; ayrıca geçersiz 'group: hover;' satırını kaldırın.
- toggleModule fonksiyonunu <script> bloğunda tanımlayın veya inline onclick yerine addEventListener ile semantik event binding kullanın.

### Performans — 93/100

**Öneriler:**
- Eğer bu tasarım React gibi bir framework ile hayata geçirilirse, curriculum listesindeki accordion yapısı için `key` özelliğinde index kullanımından kaçınılmalıdır.
- backdrop-filter kullanımı sınırlı tutulmalı; video-duration-badge yerine statik yarı-saydam arka plan tercih edilebilir, bu düşük GPU'lu cihazlarda daha iyi performans sağlar.
- Akordeon animasyonunda grid-template-rows yerine max-height transition kullanımı bazı tarayıcılarda daha performanslı olabilir, ancak mevcut Grid yaklaşımı da kabul edilebilir bir modern tekniktir.

### Güvenlik — 90/100

**Öneriler:**
- Dinamik veri veya kullanıcı girdisi eklenecekse, içerik render edilirken XSS koruması için DOMPurify gibi bir kütüphane kullanın.
- Production ortamında backend ile etkileşim eklendiğinde, API anahtarları veya token'lar asla frontend koduna gömülmemeli, güvenli environment variable'lar veya cookie tabanlı auth tercih edilmeli.
- Add a Content-Security-Policy meta tag (e.g. default-src 'self') to harden against potential XSS in future iterations.
- Move the inline onclick handler to an external, unobtrusive script attached via addEventListener for cleaner separation and reduced surface area.

### Sürdürülebilirlik — 49/100

**Öneriler:**
- Mantık ve sunumu ayırın: CSS'i harici dosyaya, JavaScript'i (akordeon toggle) harici scripte alıp ilgili fonksiyonları tanımlayın.
- İçerik verilerini (modüller, dersler) bir JavaScript veri yapısına (array/object) çıkarıp HTML'i bu veri üzerinden dinamik render edecek bir yapı kurun.
- Kod tamamlanmalı: eksik `</body></html>` kapanış etiketleri, `toggleModule` JavaScript fonksiyonu ve tüm modüller eklenmeli; ayrıca geçersiz `group: hover;` CSS kuralı kaldırılmalı.
- Tekrarlayan SVG ikonları ve ders öğesi yapıları `<svg><symbol>` + `<use>` yapısıyla veya harici bir veri kaynağından (JSON/dizi) template/loop ile üretilerek soyutlanmalı; magic string'ler (status-completed, status-playing, status-locked) yerine veri-attribute (data-status) ve CSS attribute selector kullanılmalı.
...
**Öneriler:**
- Video başlığının yanı sıra, video açıklaması ve etiketler gibi ekstra öğeler prompt'un sınırlarını aşsa da sorun değil; istenen minimum gereksinimler tam.
- Müfredat sidebar'ının akordiyon işlevselliği için JavaScript toggle mantığı eksik; prompt fonksiyonel bir akordiyon istememiş olsa da, daha iyi kullanıcı deneyimi için eklenebilir.
- Ensure the full HTML and the referenced `toggleModule` JavaScript function are included so the accordion is functional, not just visually styled.
- Remove the invalid `group: hover;` CSS rule and verify all HTML tags are properly closed to deliver a truly production-ready file.
