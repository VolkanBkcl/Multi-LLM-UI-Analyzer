...
|---|---|---|---|---|---|---|
| DeepSeek V4 Pro | 73 | 56 | 87 | 98 | 24 | 98 |
| Qwen3.6 Plus | 76 | 88 | 90 | 98 | 32 | 73 |
| Gemini 3.5 Flash | 77 | 84 | 89 | 97 | 42 | 75 |
...
|---|---|---|---|---|---|
| Okunabilirlik | 62 | 50 | — | **56** | Ortalama (J1-J2) |
| Performans | 95 | 78 | — | **87** | Ortalama (J1-J2) |
| Güvenlik | 100 | 95 | — | **98** | Ortalama (J1-J2) |
| Sürdürülebilirlik | 18 | 30 | — | **24** | Ortalama (J1-J2) |
...
### Okunabilirlik — 56/100

**Öneriler:**
- Modüller arasındaki ağır inline stil ve HTML kopyalamasını azaltmak için bir şablon veya JavaScript ile dinamik oluşturma kullanarak DRY ihlalini giderin.
- Inline stilleri CSS sınıflarına taşıyıp `style` etiketi içinde toplayarak HTML'in okunabilirliğini artırın ve satır uzunluğunu azaltın.
- Her ders satırı için ortak stilleri bir `<style>` bloğunda sınıflara (.lesson-item, .lesson-item--active) taşıyıp HTML'i kısaltarak satır uzunluklarını 120 karakterin altına indirin.
- Modülleri ve dersleri JS dizisinden render eden küçük bir fonksiyon ekleyerek 14 kez tekrarlanan aynı HTML bloğunu ortadan kaldırın; bu hem DRY ihlalini hem de magic değer tekrarını çözer.

### Performans — 87/100

**Öneriler:**
- Accordion açma/kapama işlemi sırasında tüm `.acc-content` öğelerini kapatmak için `querySelectorAll` kullanılıyor; modül sayısı arttığında bunu bir defa seçip döngüde işlemek yerine, yalnızca daha önce açık olanı takip eden bir değişken kullanarak gereksiz DOM taraması azaltılabilir.
- Sayfa yüklendiğinde ilk modülü açmak için kullanılan `DOMContentLoaded` içindeki kod, `toggleAccordion` fonksiyonunu çağırarak daha tutarlı ve tekrar kullanılabilir hale getirilebilir.
- Her ders öğesi için tekrar eden onmouseover/onmouseout handler'larını kaldırıp, sidebar konteynerine tek bir event delegation (mouseover/mouseout) listener'ı ekleyin; bu DOM listener sayısını önemli ölçüde azaltır.
- pulse-dot gibi sürekli animasyonları IntersectionObserver ile sadece sidebar görünürken çalıştırın; ayrıca max-height yerine height: auto ile grid-template-rows: 0fr/1fr geçişi kullanmak layout thrashing'i azaltır.

### Güvenlik — 98/100

**Öneriler:**
- Gelecekte kullanıcı girdisi eklenirse, XSS riskine karşı içerik sanitizasyonu uygulanmalıdır.
- İleride API entegrasyonu yapılırsa, HTTPS zorunlu tutulmalı ve token'lar güvenli saklanmalıdır.
- Üretime alınırsa Content-Security-Policy meta etiketi (ör. script-src 'self') ekleyerek inline event handler ve style kullanımını sınırlandırın.
- İleride dinamik içerik (kullanıcı yorumu, video URL) eklenecekse tüm girdileri DOMPurify veya textContent ile işleyerek XSS riskini önleyin.

### Sürdürülebilirlik — 24/100

**Öneriler:**
- Bileşen Mimarisi ve Veri Yönetimi: Akordeon öğeleri için manuel HTML tekrarından kaçının; müfredat verisini bir JavaScript dizisine taşıyıp, bileşeni bu veri üzerinden döngüyle render edin. Mevcut yapıda yeni bir modül eklemek için tüm HTML'i kopyalamak gerekiyor, bu da hata riskini artırır.
- Stil ve Mantık Ayrıştırması: Inline CSS ve satır içi olay yöneticileri (onmouseover/out) yerine CSS sınıflarını kullanın ve JavaScript'te `classList.toggle` ile yönetin. `toggleAccordion` fonksiyonu, stil değerlerini manuel ayarlamak yerine CSS geçişlerine güvenmeli, bu sayede sunum ve iş mantığı ayrışarak kodun okunabilirliği ve değiştirilebilirliği artar.
- Ders ve modül bloklarını <style> bloğunda tanımlanmış CSS sınıfları (.lesson, .module-header, .module-content) ile soyutla; bu sayede tek bir değişiklikle tüm listeyi güncelleyebilirsin.
- Modül ve ders verisini bir JavaScript dizisine taşıyıp render fonksiyonu ile DOM'a bas; böylece yeni modül eklemek yalnızca diziye bir öğe eklemek kadar basit olur ve test edilebilirlik artar.
...
**Öneriler:**
- Müfredat kenar çubuğundaki modül sayısı (5) başlıktaki '24 lessons' ile tam olarak eşleşmiyor; veri bütünlüğü için bunların tutarlı olması iyileştirilebilir.
- Akordeon elemanları için daha anlamsal bir HTML yapısı (ör. <details>/<summary> etiketleri) kullanılarak erişilebilirlik artırılabilir, ancak işlevsellik açısından mevcut yapı yeterlidir.
- Mobil cihazlar için @media query ekleyerek (örneğin 768px altında) sidebar'ı alta alıp video player'ı tam genişlikte göstermek gerçek responsive davranışı sağlar.
- Video player placeholder'ı gerçek bir <video> elementi ile değiştirip src, controls ve poster attribute'leri eklemek production-ready beklentisini daha da güçlendirir.
...
| Metrik | J1 | J2 | J3 | Final | Çözüm Yöntemi |
|---|---|---|---|---|---|
| Okunabilirlik | 88 | 87 | — | **88** | Ortalama (J1-J2) |
| Performans | 100 | 80 | — | **90** | Ortalama (J1-J2) |
| Güvenlik | 100 | 95 | — | **98** | Ortalama (J1-J2) |
| Sürdürülebilirlik | 15 | 48 | 32 | **32** | Medyan (J1·J2·J3) — tahkim |
| Prompt Uyumu | 80 | 65 | — | **73** | Ortalama (J1-J2) |
...
**Öneriler:**
- toggleModule fonksiyonunu tanımlayın veya eksik olduğu yorumla belirtin.
- Renkler için CSS değişkenleri kullanarak tutarlılığı ve sonraki değişikliklerde okunabilirliği artırabilirsiniz.
- Farklı kontrol ikonlarını ayırt etmek için `.control-icon--volume`, `.control-icon--settings` gibi BEM modifier'ları ekleyin; böylece hangi ikonun hangisi olduğu sınıf adından anlaşılır.
- Tekrarlanan SVG ikonlarını (play üçgeni, check işareti) `<svg><use xlink:href="#icon-play"/></svg>` ile tek bir `<defs>` bloğundan referanslayarak DRY ihlalini azaltın.

### Performans — 90/100

**Öneriler:**
- Gelecekte eklenebilecek JavaScript olay dinleyicileri için bellek sızıntısını önlemek adına cleanup yapılması önerilir.
- toggleModule fonksiyonunun tanımlı olduğundan emin olunmalı, aksi halde konsol hataları sayfa yanıt verme hızını etkileyebilir.
- Accordion animasyonu için `max-height` yerine sabit yükseklik, `grid-template-rows: 0fr → 1fr` tekniğini veya JS ile `scrollHeight` ölçümü kullanın; bu layout thrashing'i ortadan kaldırır.
- `.lesson { transition: all 0.2s }` yerine yalnızca değişen özellikleri (ör. `background, color`) listeleyerek tarayıcıya gereksiz property animasyonu yaptırmayın.

### Güvenlik — 98/100

**Öneriler:**
- İleride bu tasarıma dinamik içerik (ör. kullanıcı yorumları) eklenirse, XSS riskine karşı verilerin güvenli bir şekilde render edildiğinden emin olun (örn. DOMPurify kullanımı).
- Eğer video başlığı veya müfredat bilgileri bir API'den dinamik olarak çekilecekse, gelen verinin güvenilir kaynaktan olduğundan ve gerekiyorsa sanitize edildiğinden emin olun.
- Inline onclick kullanımı yerine addEventListener ile harici bir script bloğunda event bağlama yaparak defense-in-depth prensibini güçlendirin.
- Gelecekte dinamik içerik eklenirse, kullanıcı girdilerini innerHTML yerine textContent veya DOMPurify ile sanitize ederek render edin.

### Sürdürülebilirlik — 32/100

**Öneriler:**
- HTML, CSS ve JavaScript'i ayrı dosyalara/modüllere bölün; CSS değişkenleri ve bileşen tabanlı yapı (ör. video-player, curriculum-sidebar bileşenleri) ile modülariteyi artırın.
- Accordion mantığını inline onclick yerine bir JavaScript modülünde veya framework (React/Vue) ile state yönetimi kullanarak yönetin, veri ve SVG ikonlarını ayrı sabit dosyalara çıkarın.
- Tekrar eden renkler ve boyutlar için :root üzerinde CSS custom properties (--color-primary, --space-md vb.) tanımlayarak tema değişikliklerini tek noktadan yönetin.
- Ders ve modül verilerini JS dizisine çıkarıp render fonksiyonu ile üretin; SVG ikonlarını <symbol>/<use> ile bir defa tanımlayıp tekrar eden markup'ı ortadan kaldırın.
...
**Öneriler:**
- Video oynatıcı placeholder'ı mevcut ancak 'placeholder' kavramı biraz daha sade veya gerçek video embed ediliyormuş gibi bir görünüm beklenebilir; yine de prompt karşılanmıştır.
- Kodda inline CSS kullanılmamış, tüm CSS <style> etiketi içinde; prompt tam olarak 'inline CSS' istemişse bu biçimsel bir ihlal olabilir ancak semantik değerlendirmede etkisi yoktur.
- Kodun tamamını eksiksiz teslim edin; açılış/kapanış etiketleri ve toggleModule JavaScript fonksiyonu mutlaka dahil edilmeli.
- Üretime hazır kod kuralı gereği HTML'in tamamlandığından ve accordion aç/kapa etkileşiminin çalıştığından emin olun.
...
| Prompt Uyumu | 77 | 73 | — | **75** | Ortalama (J1-J2) |
...
**Öneriler:**
- 'group: hover;' satırı geçersiz CSS'tir, kaldırılmalı.
- Accordion için JavaScript işlevselliğinin eksik olması, kodun tam bağlamını anlamayı zorlaştırabilir; en azından bir yorum eklenmeli veya basit bir script bloğu ile desteklenmeli.
- Remove the invalid `group: hover;` declaration from `.video-placeholder` (or replace with the intended selector like `&:hover` if using a preprocessor) to avoid confusing future readers.
- Either include the `toggleModule` JavaScript function in the same file or move the click handler out of inline HTML so the markup is self-contained and readable.

### Performans — 89/100

**Öneriler:**
- grid-template-rows geçişi düşük güçlü cihazlarda hafif takılmalara yol açabilir; alternatif olarak max-height tabanlı bir çözüm değerlendirilebilir.
- Müfredat listesi çok büyük olsaydı sanallaştırma gerekirdi, ancak şu anki boyut için gereksizdir.
- Ağır `box-shadow` ve `backdrop-filter: blur(8px)` gibi pahalı görsel efektleri sınırlandırın; kart gölgesi için tek bir optimize edilmiş değer yeterli olabilir.
- Geçersiz CSS özelliklerini (örn. `.video-placeholder` içindeki `group: hover`) temizleyin ve gereksiz transition'ları kaldırarak paint/composite maliyetini azaltın.

### Güvenlik — 97/100

**Öneriler:**
- Production ortamında kullanılacaksa, sunucu tarafında Content Security Policy (CSP) başlığı eklenmesi XSS riskini azaltmaya yardımcı olur.
- İleride dinamik içerik (örneğin kullanıcı yorumu) eklenecek olursa, JSX içinde güvenli bir şekilde render edilmeli veya DOMPurify gibi bir kütüphane ile sanitize edilmelidir.
- Eğer ileride dinamik içerik (kullanıcı yorumları, AI çıktısı) eklenecekse DOMPurify veya React gibi framework'lerin otomatik escaping özelliğinden yararlanılmalıdır.
- Dış kaynaklara video veya API istekleri eklendiğinde HTTPS zorunluluğu ve CORS politikaları net olarak tanımlanmalıdır.

### Sürdürülebilirlik — 42/100

**Öneriler:**
- Tüm CSS kodunu ayrı bir stylesheet dosyasına taşıyın; bileşenleri (video oynatıcı, curriculum, ders listesi) bağımsız HTML/CSS/JS modüllerine ayırın.
- JavaScript etkileşimi için accordion toggle fonksiyonunu ayrı bir script dosyasına alın ve hata durumlarını (null kontrolü, event listener yönetimi) ekleyin.
- Ders/modül verilerini bir JS dizisinde tutun ve `renderCurriculum(data)` gibi tek bir fonksiyonla DOM'a basın; bu sayede yeni ders eklemek için HTML kopyala-yapıştır yapmak gerekmesin.
- Geçersiz `group: hover;` kuralını kaldırın, `toggleModule` fonksiyonunu eksiksiz tanımlayın ve accordion durumunu (açık/kapalı) tek bir state nesnesi ile yönetin; ayrıca ilerleme yüzdesi gibi magic number'ları CSS değişkeni veya JS sabiti olarak dışa çıkarın.
...
**Öneriler:**
- Tarayıcı otomatik kaydırma çubuğu yerine tercih edilirse, özel kaydırma çubuğu stilleri görsel bütünlük açısından iyileştirilebilir.
- Müfredat öğelerine tıklanabilirlik ve aktif durum gösterimi daha belirgin hale getirilerek kullanıcı deneyimi artırılabilir.
- Code is truncated — ensure the full HTML including the closing </body></html> tags and the toggleModule JavaScript function is delivered without cutoff.
- Add ARIA attributes (aria-expanded, role='region') to the accordion modules to improve accessibility and align better with production-ready expectations.
