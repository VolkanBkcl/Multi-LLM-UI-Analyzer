...
|---|---|---|---|---|---|---|
| DeepSeek V4 Pro | 71 | 39 | 88 | 98 | 33 | 98 |
| Qwen3.6 Plus | 76 | 80 | 88 | 96 | 42 | 74 |
| Gemini 3.5 Flash | 79 | 88 | 91 | 98 | 46 | 72 |
...
|---|---|---|---|---|---|
| Okunabilirlik | 35 | 42 | — | **39** | Ortalama (J1-J2) |
| Performans | 95 | 80 | — | **88** | Ortalama (J1-J2) |
| Güvenlik | 100 | 95 | — | **98** | Ortalama (J1-J2) |
| Sürdürülebilirlik | 30 | 35 | — | **33** | Ortalama (J1-J2) |
...
### Okunabilirlik — 39/100

**Öneriler:**
- CSS kurallarını ve stilleri ayrı bir `<style>` bloğunda veya harici dosyada toplayarak tekrarı azaltın ve okunabilirliği artırın.
- JavaScript mantığını HTML özniteliklerinden (`onclick`, `onmouseover`) çıkarıp ayrı bir `<script>` içinde `addEventListener` ile yöneterek yapıyı basitleştirin.
- Accordion butonu ve ders öğesi için tekrar eden inline stilleri ortak bir CSS sınıfına (.module-btn, .lesson-item) çıkararak DRY ihlalini giderin ve satır uzunluklarını düşürün.
- `<style>` etiketlerini `<head>` içine taşıyın; magic değerler (renk paleti, border-radius) için CSS değişkenleri (--primary, --radius) tanımlayarak okunabilirliği ve tutarlılığı artırın.

### Performans — 88/100

**Öneriler:**
- Üretim ortamı için CSS animasyonlarında GPU hızlandırmalı transform/opacity tercih edilmiş, ancak `max-height` geçişlerinde `scrollHeight` hesaplaması yalnızca tıklama anında yapıldığı için performansa etkisi ihmal edilebilir düzeydedir. Daha büyük ölçekli uygulamalarda `requestAnimationFrame` veya `will-change` kullanımı düşünülebilir.
- Başka bir iyileştirmeye şu an için ihtiyaç yoktur; kod hali hazırda performans açısından temiz ve verimlidir.
- Accordion animasyonlarında 'max-height' yerine 'transform: scaleY' + 'opacity' veya sabit bir max-height değeri kullanarak reflow tetiklemesini önleyin.
- Çok sayıda elementteki 'transition: all' ifadelerini yalnızca değişen özellikleri (ör. 'background', 'transform', 'box-shadow') kapsayacak şekilde daraltarak gereksiz paint maliyetini azaltın.

### Güvenlik — 98/100

**Öneriler:**
- Even though this code has no security risks, avoid defining styles directly in HTML attributes (e.g., onmouseover='this.style.background=...') for larger projects — consider CSS classes to reduce maintenance overhead and enforce Content Security Policy more easily.
- Ensure any future dynamic content (video thumbnails, user-provided titles) is properly sanitized before injection into the DOM.
- İleride dinamik içerik (kullanıcı adı, yorum, başlık) eklenecekse tüm kullanıcı girdilerini textContent veya DOM API ile enjekte edin, innerHTML'den kaçının.
- API entegrasyonu eklendiğinde fetch çağrılarını HTTPS zorunluluğu ve uygun Content-Security-Policy header'ı ile sıkılaştırın.

### Sürdürülebilirlik — 33/100

**Öneriler:**
- Accordion modülleri ve ders öğeleri için bir veri yapısı oluşturup (JavaScript array of objects), bu veriyi dinamik olarak render eden bir fonksiyon veya bileşen kullanarak kod tekrarını ortadan kaldırın.
- Stil tanımlarını <style> etiketleri içinde CSS sınıflarına dönüştürerek ve olay yönetimini addEventListener ile ayırarak HTML içi stil/script karışıklığını giderin.
- Accordion modülü ve ders öğesi için tek bir CSS sınıfı (.module-button, .lesson-item) tanımlayıp tek satırda yeniden kullanın; her dersteki tekrarlanan inline style'ları ortadan kaldırın.
- Modülleri ve dersleri bir JavaScript dizisinden (data array) render edin; böylece yeni modül/eklemek için tek bir veri kaynağını düzenlemek yeterli olur ve DOM tekrarı ortadan kalkar.
...
**Öneriler:**
- Sağ taraftaki '24 lessons' etiketiyle verilen toplam ders sayısı, gösterilen ders sayısıyla birebir uyuşmuyor; bu minör şekilde giderilebilir.
- Özel kaydırma çubuğu stilleri body'deki genel stili ezebilir; sadece sağ sidebar'ı hedefleyen daha spesifik bir seçici kullanılabilir.
- Mobil ekranlar için @media query breakpoint'leri ekleyerek tam responsive davranışı garanti edin (örn. 768px altında sidebar üstte olacak şekilde).
- Tüm CSS'i gerçekten inline yapmak için <style> blokundaki scrollbar ve keyframe kurallarını ilgili elemanlara inline style veya SVG/satır-içi animasyon alternatifi ile taşıyın.
...
| Metrik | J1 | J2 | J3 | Final | Çözüm Yöntemi |
|---|---|---|---|---|---|
| Okunabilirlik | 80 | 80 | — | **80** | Ortalama (J1-J2) |
| Performans | 100 | 76 | 88 | **88** | Medyan (J1·J2·J3) — tahkim |
| Güvenlik | 100 | 92 | — | **96** | Ortalama (J1-J2) |
| Sürdürülebilirlik | 25 | 58 | 42 | **42** | Medyan (J1·J2·J3) — tahkim |
| Prompt Uyumu | 79 | 69 | — | **74** | Ortalama (J1-J2) |
...
**Öneriler:**
- Akordiyon işlevselliği için eksik olan toggleModule JavaScript fonksiyonu eklenmeli; aksi halde kodun çalışma mantığı anlaşılamaz.
- Tekrar eden satır içi SVG simgeleri CSS sınıflarına taşınarak HTML okunabilirliği artırılabilir.
- `toggleModule` JavaScript fonksiyonunu ekleyin veya her module-header'a bağımsız bir addEventListener ile davranış kazandırın, böylece HTML'in işlevsellikle bütünlüğü sağlansın.
- Tekrar eden lesson SVG ikonlarını (play/check/lock) sadeleştirmek için CSS pseudo-element + data-attribute yaklaşımına geçin; hem DRY ihlalini azaltır hem de okumayı kolaylaştırır.

### Performans — 88/100

**Öneriler:**
- Canlı video verisi veya API entegrasyonu eklendiğinde, veri çekme işlemleri render döngüsünden izole edilmeli ve büyük veri setleri için sanallaştırma düşünülmelidir.
- Herhangi bir JavaScript mantığı yoktur; olası iyileştirme için saf CSS kullanılmıştır, ancak ileride eklenecek event listener'ların bellek sızıntısını önlemek için temizlendiğinden emin olunmalıdır.
- Accordion için `max-height` geçişi yerine içeriğin gerçek yüksekliğini hesaplayan `height: auto` ile CSS `grid-template-rows: 0fr -> 1fr` tekniğini kullanın; bu layout thrashing'i ortadan kaldırır.
- Hover sırasında tetiklenen box-shadow ve transform animasyonlarını `will-change: transform` ile sınırlandırarak compositing katmanına taşıyın ve repaint maliyetini düşürün.

### Güvenlik — 96/100

**Öneriler:**
- Uygulama ileride genişletildiğinde, video başlığı veya eğitmen bilgisi gibi alanlar kullanıcı girişiyle doldurulacaksa olası XSS riskine karşı bu verilerin DOMPurify gibi bir kütüphane ile sanitize edilmesi gerekir.
- Harici video oynatıcı entegrasyonunda video URL'leri kullanılacaksa, URL'lerin yalnızca güvenilir kaynaklardan geldiğinden ve HTTPS üzerinden sunulduğundan emin olunmalıdır.
- Move inline onclick handlers (e.g., toggleModule) to an external JS file with addEventListener to improve CSP compatibility.
- Add a Content-Security-Policy meta tag and Subresource Integrity attributes when introducing any external scripts or CDNs in the future.

### Sürdürülebilirlik — 42/100

**Öneriler:**
- HTML, CSS ve JavaScript'i ayrı dosyalara çıkararak bileşenleri (video-player, curriculum, lesson-item) fonksiyonel modüllere ayırın.
- Satır içi onclick yerine addEventListener veya bir modül kapsüllemesi kullanarak etkileşim mantığını sunum katmanından ayırın.
- Renk paleti, tipografi ve spacing değerlerini CSS custom properties (--color-primary, --color-text-muted vb.) olarak token halinde tanımlayın; böylece tema veya marka değişiklikleri tek yerden yönetilir.
- `onclick="toggleModule(this)"` gibi inline handler'lar yerine tek bir `addEventListener` ile event delegation kullanın; böylece HTML–JS bağımlılığı azalır ve yeni modüller için tekrar tekrar attribute yazmak gerekmez.
...
**Öneriler:**
- Sol taraftaki video başlığı, talep edilen şekilde video oynatıcının altında yer alıyor, ancak 'video title' ile birlikte eklenen metadatalar (görüntülenme, yorum vb.) gereksinimde açıkça belirtilmese de uyumu bozmuyor.
- Sağ taraftaki curriculum sidebar zorunlu tutulan accordion yapısını tam olarak sağlıyor ve kaydırılabilir yapıdadır, ek bir iyileştirme olarak mobilde accordion geçişlerine erişilebilirlik özellikleri eklenebilir.
- Kodun tamamını eksiksiz teslim edin: tüm modülleri, kapanış HTML etiketlerini ve toggleModule JavaScript fonksiyonunu dahil edin.
- toggleModule fonksiyonunu, accordion açılma/kapanma davranışını düzgün çalıştıracak şekilde module-content ve module-header class'larını toggle edecek şekilde tanımlayın.
...
| Prompt Uyumu | 80 | 63 | — | **72** | Ortalama (J1-J2) |
...
**Öneriler:**
- Remove the invalid 'group: hover;' line from the .video-placeholder class to prevent confusion.
- Consider adding a short comment explaining the CSS Grid accordion animation technique, as it may not be immediately obvious to all developers.
- Tanımsız `group: hover;` CSS özelliğini kaldırın; bu geçersiz bir deklarasyondur ve okuyucuyu yanıltır. SVG ikonları için `<symbol>` + `<use>` desenini kullanarak tekrar eden path'leri ortadan kaldırın.
- `onclick="toggleModule(this)"` ile çağrılan JavaScript fonksiyonu tanımını dosyaya ekleyin veya accordion davranışını saf CSS (örn. `<details>`/`<summary>`) ile değiştirerek bağımlılığı ortadan kaldırın.

### Performans — 91/100

**Öneriler:**
- Sayfa statik olduğundan performans yüksektir; ancak eksik `toggleModule` fonksiyonu tarayıcı hatasına yol açabilir, bu da kullanıcı deneyimini dolaylı olarak etkiler. Fonksiyonu tanımlayın.
- Mevcut yapıda performans sorunu yok; gelecekte dinamik içerik eklenirse büyük listeler için sanallaştırma (virtualization) düşünülebilir.
- SVG ikonlarını `<symbol>`/`<use>` ile bir sprite içinde tanımlayıp tekrar kullanarak HTML boyutunu ve parse süresini azaltın.
- Video-duration-badge gibi küçük öğelerdeki `backdrop-filter: blur(8px)` yerine yarı-saydam düz arka plan kullanarak GPU maliyetini düşürün.

### Güvenlik — 98/100

**Öneriler:**
- Kod şu an salt statik HTML/CSS olduğu için güvenlik riski taşımamaktadır; ileride JavaScript eklenirse XSS ve input doğrulama kontrolleri yapılmalıdır.
- Herhangi bir form veya kullanıcı etkileşimi eklenmeden önce OWASP ilkeleri göz önünde bulundurulmalıdır.
- İleride dinamik içerik eklenecekse tüm kullanıcı girdilerini escape/sanitize edin ve DOMPurify benzeri bir kütüphane kullanın.
- Bir Content-Security-Policy meta etiketi ekleyerek inline script kullanımını sınırlandırın ve harici kaynak yüklemelerini kısıtlayın.

### Sürdürülebilirlik — 46/100

**Öneriler:**
- Bileşen tabanlı bir mimariye geçin (React/Vue) ve her UI parçasını (video alanı, sidebar, ders öğeleri) ayrı bileşenlere ayırın.
- Accordion aç/kapat mantığını ayrı bir JS dosyasına taşıyın, state yönetimi ekleyin ve hata/boş durumları tanımlayın.
- Tekrarlayan SVG ikonlarını tek bir <svg><symbol> sprite'ında tanımlayıp <use href='#icon-check'/> ile çağırarak HTML duplikasyonunu ortadan kaldırın; modül/lesson listesini bir JS dizisinden render ederek veri-odaklı hale getirin.
- Geçersiz 'group: hover' kuralını kaldırın, kodu tamamlayın (eksik modul kapanışı), toggleModule fonksiyonunu açıkça tanımlayın ve progress oranı/lesson sayısı gibi değerleri CSS değişkeni veya data-attribute olarak dışa çıkarın.
...
**Öneriler:**
- Tüm semantik kısıtlamalar (ör. 'sadece kod', 'açıklama yapma') zaten karşılandığı için iyileştirme önerisi bulunmamaktadır.
- Prompt'ta üretime hazır (production-ready) ve estetik vurgusu yapılmış; mevcut kod bu beklentiyi karşılıyor.
- Kodun tamamını eksiksiz üretin: tüm modül/lesson öğelerini yazıp `</body></html>` ile kapatın, böylece gerçekten render edilebilir olsun.
- `onclick="toggleModule(this)"` için ilgili JavaScript fonksiyonunu (`<script>` bloğunda) tanımlayarak akordeon açılıp/kapanma davranışını çalışır hale getirin.
