...
|---|---|---|---|---|---|---|
| DeepSeek V4 Pro | 79 | 82 | 77 | 95 | 42 | 98 |
| Qwen3.6 Plus | 86 | 79 | 80 | 96 | 78 | 98 |
| Gemini 3.5 Flash | 75 | 70 | 73 | 95 | 39 | 98 |
...
| Sürdürülebilirlik | 42 | 42 | — | **42** | Ortalama (J1-J2) |
...
**Öneriler:**
- Uzun inline stil ve sınıf birleştirme mantığı (ör. left/top calc ifadeleri) ayrı bir yardımcı fonksiyona taşınabilir.
- Eylem butonlarının disabled durumu için tekrar eden mantık (selectedTables.length kontrolü) sabit veya ortak bir fonksiyonla sadeleştirilebilir.
- Bileşeni küçük parçalara ayırın: TableCard, StatusLegend, ControlBar, ve ortak bir Modal bileşeni oluşturarak Payment/Split Bill modallarındaki tekrarı ortadan kaldırın.
- Konumlandırma için kullanılan magic pixel değerlerini (40/48/56/72) ve 2000ms timeout gibi sabitleri anlamlı sabitler olarak (ör. TABLE_SIZE, FEEDBACK_DURATION_MS) tanımlayın.

### Performans — 77/100

**Öneriler:**
- `getTableStyle` fonksiyonunu `useMemo` içine alın veya her tablo için stil objesini önceden hesaplayarak memoize edin, ayrıca inline stil hesaplamalarını da önbelleğe alın.
- Büyük grid hücreleme işlemini (`Array.from`) render dışında sabit bir veri olarak tanımlayın veya `useMemo` ile sararak tekrar render'larda hesaplamayı önleyin.
- showFeedback içindeki setTimeout için useRef ile timer ID tutun ve component unmount olduğunda veya yeni feedback gösterildiğinde clearTimeout çağırın.
- selectedTables.includes(table.id) tekrarını önlemek için selectedTables'i Set'e çevirin veya tables map'inde tek seferde selected kontrolü yapın.

### Güvenlik — 95/100

**Öneriler:**
- Masa etiketlerini server'dan alınan veriyle oluşturuyorsanız, `table.label` gibi alanları JSX içinde doğrudan gösterdiğiniz için bu değerlerin XSS içermediğinden emin olun; mümkünse server tarafında sanitize edilmiş hallerini kullanın.
- Kullanıcıya gösterilen hata mesajları veya state değerleri hassas veri içermemeli; mevcut yapıda yalnızca generic mesajlar olduğundan sorun yok, ancak ileride API hata yanıtlarını doğrudan kullanıcıya yansıtmaktan kaçının.
- Gelecekte gerçek ödeme entegrasyonu eklenecekse kart bilgisi (PAN/CVV) hiçbir zaman client state'inde tutulmamalı, PCI-DSS uyumlu tokenizasyon (Stripe Elements gibi iframe tabanlı) kullanılmalıdır.
- API çağrıları eklendiğinde fetch URL'leri için HTTPS zorunluluğu, hata mesajlarında stack trace sızıntısının engellenmesi ve CSP header'ı konfigürasyonu eklenmelidir.

### Sürdürülebilirlik — 42/100

**Öneriler:**
- Bileşenleri küçük parçalara ayırın: Table, TableGrid, ControlBar, PaymentModal, SplitBillModal gibi alt bileşenler oluşturun.
- İş mantığını custom hook'lara taşıyın (ör. useTableManagement, useTableSelection) ve state yönetimini ayrıştırarak test edilebilirliği artırın.
- Bileşeni Table, Modal, ActionButton, ControlBar, Header gibi alt bileşenlere ayırın; merge/split/payment iş mantığını 'useFloorPlan' gibi bir custom hook'a taşıyın.
- Table shape ve positioning mantığını bir konfigürasyon nesnesine (örn. SHAPE_STYLES) çıkararak magic number ve string karşılaştırmalarını ortadan kaldırın.
...
**Öneriler:**
- Masa seçimi için daha belirgin bir görsel geri bildirim eklenebilir (örneğin seçili masalarda animasyon).
- Kontrol çubuğu butonlarına erişilebilirlik için aria-label eklenmesi iyi olur.
- Sticky bar'ın küçük ekranlarda butonların taşmasını önlemek için mobil için responsive (kısaltılmış etiket veya ikon-only) varyantı eklenebilir.
- Tablo konumlandırması için hard-coded pixel offset'ler yerine CSS grid satır/sütun indeksleri (gridColumn/gridRow) kullanılarak daha sağlam bir yerleşim elde edilebilir.
...
| Sürdürülebilirlik | 80 | 75 | — | **78** | Ortalama (J1-J2) |
...
**Öneriler:**
- Emoji simgeleri (🟢, 🔴) statik SVG ikonlar ile değiştirin veya en azından aria-label ekleyerek erişilebilirliği artırın.
- Anlamlı sabitler tanımlayın: Örneğin maksimum seçim limiti (3) için MAX_SELECTABLE_TABLES sabiti kullanın.
- Modalları (`SplitBillModal`, `MergeTablesModal`, `PaymentModal`) ayrı bileşenler olarak çıkararak `RestaurantFloorPlan` ana bileşenini ~150 satırın altına indirin.
- Tekrarlanan buton stilleri için ortak bir `ActionButton` bileşeni tanımlayarak DRY ihlalini giderin ve stil değişikliklerini tek noktadan yönetilebilir hale getirin.

### Performans — 80/100

**Öneriler:**
- Bildirimlerde kullanılan 'setTimeout' işlemi için, bileşen unmount olduğunda bellek sızıntısını önlemek adına 'clearTimeout' ile bir cleanup mekanizması eklenmelidir.
- 'useCallback' hook'unun 'handleSelectTable' fonksiyonuna 'showNotification' bağımlılığı eklenmeli, aksi takdirde stale closure riski oluşabilir.
- showNotification'daki setTimeout için bir ref tutun ve useEffect cleanup'ında (veya bildirim kapandığında) clearTimeout çağırın; ayrıca AbortController benzeri bir isMounted pattern ile unmount sonrası setState'i engelleyin.
- TableCard bileşenini React.memo ile sarın; ayrıca stats hesaplamasını tek bir reduce döngüsünde yaparak dört ayrı filter pass'ini ortadan kaldırın.

### Güvenlik — 96/100

**Öneriler:**
- Gelecekteki API entegrasyonlarında HTTPS kullanın ve token'ları HttpOnly cookie ile yönetin.
- Kullanıcıdan alınan herhangi bir dinamik içerik (ör. yorum, isim) için DOMPurify ile sanitizasyon uygulayın.
- Ödeme işlemi şu an yalnızca client-side simülasyon; gerçek bir entegrasyonda ödeme bilgilerinin PCI-DSS uyumlu bir ödeme sağlayıcısına (Stripe/Adyen) yönlendirilmesi ve kart verilerinin hiçbir zaman client'ta tutulmaması gerekir.
- İleride API entegrasyonu eklenecekse hassas veri (müşteri bilgisi, ödeme tutarı) için HTTPS zorunluluğu, hata mesajlarında stack trace sızıntısının engellenmesi ve rate limiting uygulanmalıdır.

### Sürdürülebilirlik — 78/100

**Öneriler:**
- Ana bileşendeki durum yönetimi ve iş mantığını (tablo seçimi, bildirim, ödeme vb.) özel hook'lara (useTableSelection, useNotification) taşıyarak bileşen boyutunu küçültün ve test edilebilirliği artırın.
- Modal içeriklerini (SplitBill, MergeTables, Payment) ayrı bileşenlere çıkararak ana bileşenin okunabilirliğini ve değiştirilebilirliğini iyileştirin.
- Ana 'RestaurantFloorPlan' bileşenindeki state ve iş mantığını 'useTableSelection' ve 'useNotifications' gibi custom hook'lara ayırarak bileşeni sunum katmanına indirgeyin.
- 'activeModal' için kullanılan magic string'leri (örn. 'splitBill') bir constants dosyasında enum/sabit olarak tanımlayın ve modal içeriklerini ayrı bileşenlere (SplitBillModal, MergeTablesModal, PaymentModal) bölerek modifiability'yi artırın.
...
**Öneriler:**
- Prompt'ta belirtilen 'maintenance' durumu için ekstra bir renk kodu eklenmiş; istenen üç renk dışında ek durumlar eklenmiş ancak bu kritik bir uyumsuzluk değil.
- Üretim (production-ready) kodu beklendiği için placeholder olmaması olumlu; tüm işlevler çalışır durumda.
- Bottom bar için `sticky bottom-0` semantiği `fixed` yerine daha açık olurdu (görsel olarak aynı sonuç verse de).
- Durum rengini yalnız kenarlık/ikon yerine, prompt'taki gibi masa gövdesinin ana rengi olarak daha baskın uygulamak (ör. `bg-emerald-500` ile dolu kart) sadakati artırır.
...
| Metrik | J1 | J2 | J3 | Final | Çözüm Yöntemi |
|---|---|---|---|---|---|
| Okunabilirlik | 68 | 72 | — | **70** | Ortalama (J1-J2) |
| Performans | 70 | 75 | — | **73** | Ortalama (J1-J2) |
...
**Öneriler:**
- Devasa bileşeni TableGrid, TableCard, ControlBar, PaymentModal gibi alt bileşenlere bölerek SRP’yi sağlayın ve dosya uzunluğunu 150 satırın altına indirin.
- JSX’teki iç içe ternary ifadeleri kaldırın; her durum için ayrı bir değişken veya yardımcı bileşen kullanarak koşullu renderı düzleştirin.
- RestaurantFloorPlan bileşenini TableCard, TableDetailPanel, ActionBar, PaymentModal, SplitBillModal gibi alt bileşenlere ayırarak ~150 satırın altına indirin; her alt bileşen tek bir sorumluluğa odaklansın.
- Vergi oranı (TAX_RATE = 0.15), zaman aşımı (PAYMENT_SUCCESS_DELAY_MS = 1500) ve varsayılan fatura tutarı gibi magic number'ları dosya başında açıklayıcı sabitler olarak çıkarın; useEffect importunu kaldırın.

### Performans — 73/100

**Öneriler:**
- Icons nesnesini bileşen dışında tanımlayarak veya `React.memo` ile sararak her render'da yeni referans oluşmasını engelleyin.
- Callback'leri (`handleStartSplitMode`, `cancelMultiSelect` vb.) ve özellikle `setTimeout` içeren `finalizePayment` fonksiyonunu `useCallback` ile memoize edin; ayrıca `setTimeout`'un referansını saklayarak bileşen unmount olduğunda `clearTimeout` ile temizleyin.
- finalizePayment içindeki setTimeout'ı useEffect + cleanup (clearTimeout) ile sarmalayın; modal kapandığında veya bileşen unmount olduğunda tetiklenmesini engelleyin.
- Footer'daki seçili masa isimleri için selectedNames = useMemo(...) ekleyerek her render'da tekrar eden tables.find() çağrılarını önleyin; ayrıca stats'ı tek reduce döngüsünde hesaplayın.

### Güvenlik — 95/100

**Öneriler:**
- İleride gerçek bir API'den gelen dinamik veriler (özellikle `server`, `name` gibi alanlar) kullanılırsa, XSS koruması için bu değerlerin React'in varsayılan escape mekanizmasına güvenerek JSX'te doğrudan render edilmesine devam edilmeli, ancak `dangerouslySetInnerHTML` kullanılmamalıdır.
- Sticky control bardaki `selectedTableIds.map(...)` ile masa adlarının birleştirilmesi güvenli birleştirme yöntemi olup herhangi bir risk taşımamaktadır, fakat sunucu taraflı validasyon olmadığı hatırlatılmalıdır.
- Gerçek ödeme akışı (kart bilgisi) eklenecekse PCI-DSS uyumlu, tokenizasyon yapan bir ödeme sağlayıcısı (Stripe Elements vb.) kullanın ve hiçbir koşulda kart verisini React state'inde tutmayın.
- Gelecekte kullanıcı girdisi (örn. rezervasyon adı, not) eklenecekse tüm dinamik içerikler JSX üzerinden render edilmeli; HTML/URL girdileri için DOMPurify ve URL doğrulaması uygulanmalıdır.

### Sürdürülebilirlik — 39/100

**Öneriler:**
- Bileşenleri küçük, bağımsız parçalara ayırın (ör. TableCard, SectionTabs, StatsBar, ControlBar, SplitBillModal, PaymentModal). Her biri tek sorumluluk taşımalı.
- Durum sabitlerini ('vacant', 'occupied', 'reserved') ve iş mantığını ayrı bir constants.ts ve custom hook'lara (useTableManagement, useMultiSelect) taşıyarak magic string'leri ve tekrarı azaltın.
- Modüler yapıyı güçlendirin: TableCard, StatsCard, ControlBar, SplitBillModal ve PaymentModal gibi alt bileşenlere ayırın; iş mantığını (handleTableClick, executeMerge, finalizePayment) custom hook'lara (useFloorPlan) taşıyın.
- Durum ve aksiyon sabitlerini merkezileştirin: STATUS, ACTION gibi enum sabitlerini ayrı bir constants.ts dosyasında tutun; renkleri ve status stillerini tek bir lookup objesine (STATUS_STYLES) çıkararak tekrarlı if/else bloklarını ortadan kaldırın.
...
**Öneriler:**
- Merge Tables mod/eylemi tam bir akış sunmuyor (örneğin seçim iptali sonrası durum net değil). Kullanıcı deneyimi açısından daha net bir onay modalı veya görsel geri bildirim eklenebilir.
- Tablo durumlarının çevrimiçi dinamiğini vurgulamak için statik mock veri yerine gerçek bir zamanlayıcı (örneğin occupied süresi güncelleyen) mantığı eklenebilir.
- Merge işlemi sonrası birleştirilen masaları 'vacant' yerine görsel olarak gizlemek/iptal etmek floor plan bütünlüğünü artırır.
- Split Bill modalında 'Custom' ve 'Split by Item' seçenekleri alert ile sınırlı kalmış; tam fonksiyonel bir item-atama ekranı eklenebilir.
