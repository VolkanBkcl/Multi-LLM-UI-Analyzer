...
|---|---|---|---|---|---|---|
| DeepSeek V4 Pro | 80 | 80 | 73 | 98 | 51 | 99 |
| Qwen3.6 Plus | 83 | 82 | 65 | 95 | 74 | 99 |
| Gemini 3.5 Flash | 77 | 73 | 80 | 96 | 39 | 98 |
...
| Performans | 78 | 68 | — | **73** | Ortalama (J1-J2) |
| Güvenlik | 100 | 95 | — | **98** | Ortalama (J1-J2) |
| Sürdürülebilirlik | 60 | 42 | — | **51** | Ortalama (J1-J2) |
...
**Öneriler:**
- Uzun ternary zincirleri (shapeClasses ve absolute pozisyon hesaplamaları) okunabilirliği artırmak için yardımcı fonksiyonlara veya obje eşleştirmelerine dönüştürülebilir.
- 'mergedGroupId' gibi string sabitler daha anlamlı bir değişkenle tanımlanarak magic string kullanımı azaltılabilir.
- Küçük bir iyileştirme: splitBillTarget için 'parts' parametresi anlamlı bir adla (örn. numberOfWays) geçilebilir.
- Extract the duplicated modal wrapper structure (backdrop, header, close button, action buttons) into a reusable Modal component to eliminate the copy-paste between Payment and Split Bill modals.
- Replace the magic pixel offsets in the inline style (40px, 48px, 56px, 72px) with a SHAPE_OFFSETS map keyed by shape type, or compute the offset from the table's actual rendered width to decouple positioning from magic numbers.

### Performans — 73/100

**Öneriler:**
- "handleUnmerge" fonksiyonunu useCallback ile sarmalayarak gereksiz yeniden render'ları önleyin.
- "getTableStyle" fonksiyonunun sonucunu useMemo ile memoize edin, özellikle sınıf adı hesaplamaları her render'da tekrarlanmasın.
- showFeedback'taki setTimeout'u useEffect ref veya useRef ile izleyip bileşen unmount'unda clearTimeout ile temizleyin.
- selectedTables.includes(table.id) gibi tekrar eden aramaları render öncesi bir Set'e çevirerek O(1) yapın; tables.map içinde getTableStyle sonucunu ve inline style objesini useMemo ile memoize edin.

### Güvenlik — 98/100

**Öneriler:**
- İleride masa durumları gibi dinamik veriler dış kaynaktan alınırsa, oluşturulan mesajlar için DOMPurify gibi sanitasyon kullanılması önerilir.
- Gerçek ödeme işlemleri eklenecekse, PCI-DSS uyumluluğu ve HTTPS zorunluluğu sağlanmalıdır.
- Eğer ileride tablo verileri bir API'den gelecekse, sunucu tarafında authorization kontrolleri ekleyin; ödeme ve bölüştürme gibi hassas işlemler sadece yetkili kullanıcılar tarafından yapılabilmelidir.
- Gelecekte kart bilgisi veya müşteri PII verisi işlenecekse, bu verilerin state'te tutulmaması ve console'a yazılmaması için açık bir policy belirleyin (örneğin, hassas state'leri loglama sırasında maskeleyen bir helper kullanın).

### Sürdürülebilirlik — 51/100

**Öneriler:**
- Bileşeni Table, ControlBar, Modal gibi alt bileşenlere ayırarak SRP'ye uygun hale getirin.
- Durum yönetimi ve iş mantığını özel hook'lara (useTableSelection, useMerge vb.) taşıyarak sunum katmanından ayırın ve test edilebilirliği artırın.
- Bileşeni parçalara ayırın: `<Table>`, `<TableGrid>`, `<ControlBar>`, `<PaymentModal>`, `<SplitBillModal>`, `<Header>`, `<Legend>` gibi alt bileşenler çıkarın ve iki modal için ortak bir `<Modal>` sarmalayıcı soyutlayın.
- İş mantığını (handleMergeTables, handleSplitBill, handleProcessPayment, toggleTableSelection) özel hook'lara (ör. `useTableActions`) taşıyarak sunum katmanından ayırın ve status türlerini string literal yerine `as const` ile tanımlanmış tip/union'a çevirerek tip güvenliği sağlayın.

### Prompt Uyumu — 99/100

- Programatik: 100/100 · Semantik: 97/100
- İhlaller: yok

**Öneriler:**
- Masa konumlandırma hesaplaması bazı ekran boyutlarında kaymalara neden olabilir; responsive grid için daha esnek bir CSS transform veya grid-column/row yaklaşımı düşünülebilir.
- İşlem butonlarının disabled mantığı doğru olsa da, kullanıcıya neden seçim yapması gerektiğini gösteren görsel ipuçları (tooltip veya hafif bir animasyon) eklenerek kullanıcı deneyimi geliştirilebilir.
- Split bill modal could show itemized bill amounts rather than a fixed $148.50 to be more realistic for a production system.
- Tables' status changes (vacant→occupied, reserved→vacant) could be exposed via UI to make the interface truly dynamic rather than relying only on action buttons.
...
| Performans | 90 | 58 | 65 | **65** | Medyan (J1·J2·J3) — tahkim |
| Güvenlik | 100 | 90 | — | **95** | Ortalama (J1-J2) |
...
**Öneriler:**
- Sabit 3 değerini MAX_SELECTED_TABLES gibi anlamlı bir sabitle tanımlayarak magic number kullanımını giderin ve yönetilebilirliği artırın.
- Payment Modal’da toplam tutar hesaplama mantığı tekrarlanıyor; bu hesaplamayı useMemo içinde `selectedTotal` değişkenine alarak hem tekrarı azaltabilir hem de JSX içindeki uzun satırları kısaltabilirsiniz.
- RestaurantFloorPlan bileşenini StickyBottomBar, Notification, StatsBar, TableGrid gibi alt bileşenlere bölerek 300 satır sınırını aşmayı önleyin.
- showNotification ve TIMEOUT_MS / MAX_SELECTABLE_TABLES gibi magic number'ları bileşen dışında sabit olarak tanımlayın; showNotification'ı useCallback içinde kullanmadan önce bildirim mantığını setSelectedTables'tan ayırın.

### Performans — 65/100

**Öneriler:**
- Modal bileşenleri için React.lazy + Suspense ile kod bölme (code splitting) uygulayarak ilk yükleme süresini azaltabilirsiniz.
- TableCard bileşeni her render'da yeni onClick fonksiyonu alıyor, fakat useCallback içinde handleSelectTable sabit olduğu için ekstra memoization gerekmiyor; yine de TableCard'ı React.memo ile sarmak, parent state değiştiğinde gereksiz render'ları önleyebilir.
- `TableCard`'ı `React.memo` ile sarıp prop referanslarını (`handleSelectTable`) stabilize edin; ayrıca `showNotification` fonksiyonunu `useCallback` ile sarmalayıp `handleSelectTable`'ın dependency array'ine ekleyin (stale closure çözümü).
- `showNotification` içindeki `setTimeout`'ı `useRef` ile saklayın ve component unmount'unda veya yeni bildirimde `clearTimeout` ile temizleyin; bu bellek sızıntısını ve unmount-sonrası state update hatalarını önler.

### Güvenlik — 95/100

**Öneriler:**
- İleride API entegrasyonu veya kullanıcı girişi eklenirse, HTTPS zorunlu tutulmalı ve token'lar httpOnly cookie ile saklanmalıdır. Kullanıcıdan gelen veriler render edilirken React'in varsayılan XSS koruması kullanılmalı, dangerouslySetInnerHTML'den kaçınılmalıdır.
- Form alanları eklendiğinde client-side doğrulama (email formatı, uzunluk kontrolü) uygulanmalı ve backend'e gönderilen veriler sunucu tarafında da onaylanmalıdır.
- Demo için uygun olsa da, gerçek bir ortamda kimlik doğrulama (auth) ve rol tabanlı yetkilendirme ekleyerek ödeme ve masa birleştirme işlemlerini korumalısınız.
- Ödeme modalı gerçek kart verisi işleyecekse, PCI-DSS uyumlu bir ödeme sağlayıcısı (Stripe, Adyen) entegrasyonu ve tüm hassas verilerin backend'de tokenizasyonu zorunludur.

### Sürdürülebilirlik — 74/100

**Öneriler:**
- Ana bileşen (RestaurantFloorPlan) alt bileşenlere (Header, StatsBar, FloorGrid, BottomBar) bölünmeli ve tablo seçimi, bildirim gibi mantıklar özel hook'lara (useTableSelection, useNotification) taşınmalı.
- Modal içerikleri ayrı bileşenler olarak tanımlanarak ana bileşenin karmaşıklığı azaltılmalı ve test edilebilirlik artırılmalı.
- Modal içeriklerini (SplitBillModal, MergeTablesModal, PaymentModal) ayrı bileşenlere ve business logic'i (seçim yönetimi, bildirim sistemi) custom hook'lara (useTableSelection, useNotification) çıkararak ana bileşeni sadeleştirin.
- stats nesnesini STATUS_CONFIG üzerinden otomatik türetin (ör. Object.keys(STATUS_CONFIG).reduce(...)) ve setTimeout tabanlı bildirimleri useEffect cleanup'ı ile birlikte tek bir useNotification hook'unda toplayın.
...
**Öneriler:**
- Masa durumu renkleri için promptta belirtilen 'green, red, yellow' açıkça kullanılmış, ancak 'maintenance' ek durumu eklenmiş; promptta istenmeyen bir durum olduğu için ileride çıkarılabilir veya isteğe bağlı hale getirilebilir.
- Split Bill modunda farklı yöntemler gösterilmiş ama bunların işlevselliği tam olarak implemente edilmemiş; gerçek kullanımda 'Equal Split' vb. seçeneklerin çalışır hale getirilmesi prompt uyumunu güçlendirir.
- Split Bill modal could capture the actual split result (e.g., number of guests) and update the table total accordingly rather than only displaying methods.
- Process Payment flow could include a tip field and persist payment records for a more realistic restaurant workflow.
...
| Metrik | J1 | J2 | J3 | Final | Çözüm Yöntemi |
|---|---|---|---|---|---|
| Okunabilirlik | 78 | 68 | — | **73** | Ortalama (J1-J2) |
| Performans | 85 | 75 | — | **80** | Ortalama (J1-J2) |
| Güvenlik | 100 | 92 | — | **96** | Ortalama (J1-J2) |
| Sürdürülebilirlik | 35 | 42 | — | **39** | Ortalama (J1-J2) |
| Prompt Uyumu | 97 | 98 | — | **98** | Ortalama (J1-J2) |

> J3 sütunundaki "—", o metrikte iki hakemin uzlaştığını (fark ≤ 20) ve tahkimciye gidilmediğini gösterir. Uyuşmazlık olan metriklerde final skor medyan(J1, J2, J3) ile belirlenir.

## Analiz Sonuçları

### Okunabilirlik — 73/100

**Öneriler:**
- Ana bileşeni `FloorGrid`, `TableCard`, `DetailPanel`, `BottomBar`, `SplitModal`, `PaymentModal` gibi daha küçük ve odaklı bileşenlere ayırarak okunabilirliği ve bakımı kolaylaştırın.
- Kullanılmayan `useEffect` import'unu kaldırarak kod temizliğini artırın.
- Vergi oranı (0.85/0.15), varsayılan hesap (45.00) gibi sabitleri modül başında TAX_RATE, DEFAULT_BILL gibi anlamlı sabitlerle tanımlayın.
- RestaurantFloorPlan bileşenini FloorPlanGrid, TableDetailsPanel, BottomActionBar, SplitBillModal, PaymentModal gibi alt bileşenlere ayırarak SRP ihlalini giderin ve ana dosyayı ~150 satıra düşürün.

### Performans — 80/100

**Öneriler:**
- Global stats ve filteredTables hesaplamaları doğru şekilde useMemo ile sarılmış, iyi. Event handler'lar useCallback ile memoize edilebilir; örneğin handleTableClick veya updateTableStatus sık render alan bir üst bileşende alt bileşenlere prop olarak geçiliyorsa faydalı olur.
- Büyük tablo listelerinde sanallaştırma (react-window) veya pagination düşünülebilir, şu an küçük veri setinde gerekli değil.
- finalizePayment içindeki setTimeout'u useEffect+cleanup veya ref ile izleyerek component unmount sonrası state güncellemesini engelleyin.
- Sık kullanılan tables.find() çağrılarını önlemek için bir Map/useMemo ile lookup tablosu oluşturun veya selectedTableIds işlemlerini tek bir memoized selector'da birleştirin.

### Güvenlik — 96/100

**Öneriler:**
- Gelecekte eklenecek kullanıcı girdisi alanları (ör. formlar) için client-side input validation uygulanmalıdır.
- Gerçek ödeme entegrasyonu yapıldığında, kart bilgisi gibi hassas veriler state'te saklanmamalı ve PCI DSS uyumluluğu sağlanmalıdır.
- Gelecekte gerçek API entegrasyonu eklendiğinde HTTPS zorunluluğu ve input sanitization (örn. müşteri adı, rezervasyon notu) için DOMPurify benzeri bir katman ekleyin.
- Ödeme akışında gerçek kart bilgisi işlenecekse PCI-DSS uyumlu bir ödeme sağlayıcısı (Stripe Elements, iyzico) entegrasyonu yapın ve hassas veriyi asla React state'inde tutmayın.

### Sürdürülebilirlik — 39/100

**Öneriler:**
- Bileşenleri TableGrid, TableCard, BottomBar, SplitModal gibi alt bileşenlere ayırın; her biri tek sorumluluk taşısın.
- Durum sabitlerini (vacant, occupied, reserved) bir enum veya sabitler dosyasında tanımlayın; tablo seçimi ve birleştirme mantığını custom hook'lara taşıyın.
- Status (vacant/occupied/reserved) ve shape tipleri için bir `constants.ts` dosyası oluşturup renk/sınıf eşlemesini tek bir lookup objesinde toplayın; tekrarlayan if/else bloklarını kaldırın.
- Masa kartı, istatistik kartı, kontrol çubuğu butonu ve modalları ayrı bileşenlere (TableCard, StatCard, ActionButton, SplitModal, PaymentModal) bölün; merge/bill hesaplama gibi saf mantıkları `useFloorPlan` gibi bir custom hook'a taşıyarak test edilebilirlik sağlayın.
...
**Öneriler:**
- Masa konumlandırması (x, y) şu anda ızgara düzeninde yansıtılmıyor; kullanıcı promptu dinamik bir kat planı beklentisi oluştursa da kod ızgara tabanlı statik bir düzen sunuyor. Grid yerine mutlak konumlandırma veya daha belirgin bir kat planı görselleştirmesi düşünülebilir.
- Process Payment modunda ödeme yöntemi ve bahşiş seçimi butonları sadece görsel; tıklanabilir durumda olmalarına rağmen seçim durumu yönetilmiyor. Bu etkileşimlerin tamamlanması, production-ready hissini artırır.
- Masa birleştirme ve bölme aksiyonları için seçim modundayken görsel geri bildirim (örn. vurgulu çerçeve) biraz daha belirgin hale getirilebilir.
- 'Split by Item' seçeneği şu anda yalnızca alert gösteriyor; tam işlevsel bir item-atama ekranı eklenerek semantik sadakat artırılabilir.
