...
|---|---|---|---|---|---|---|
| DeepSeek V4 Pro | 80 | 79 | 79 | 98 | 44 | 98 |
| Qwen3.6 Plus | 87 | 87 | 75 | 95 | 78 | 99 |
| Gemini 3.5 Flash | 78 | 73 | 75 | 94 | 47 | 99 |
...
|---|---|---|---|---|---|
| Okunabilirlik | 78 | 80 | — | **79** | Ortalama (J1-J2) |
| Performans | 80 | 78 | — | **79** | Ortalama (J1-J2) |
| Güvenlik | 100 | 95 | — | **98** | Ortalama (J1-J2) |
| Sürdürülebilirlik | 45 | 42 | — | **44** | Ortalama (J1-J2) |
...
**Öneriler:**
- getTableStyle içindeki koşullu sınıf atamalarını küçük yardımcı fonksiyonlara veya bir obje haritasına çıkarın.
- Inline stil hesaplamalarındaki sihirli sayıları (40px, 48px vb.) açıklayıcı sabitler olarak tanımlayın.
- Extract the two modals and bottom control bar into separate components to reduce the main component length below 150 lines and improve navigability.
- Replace the magic pixel values in the position calculation with a SHAPE_DIMENSIONS lookup object (mapping shape names to width/height) so the positioning logic and style logic share a single source of truth.

### Performans — 79/100

**Öneriler:**
- `handleMergeTables`, `handleSplitBill` gibi olay işleyicileri `useCallback` ile sarılarak alt bileşenlere prop olarak geçirilirken gereksiz yeniden oluşturmalar önlenebilir.
- Bileşen içinde `useMemo` kullanılmasa da, özellikle `tables.find` ve `tables.some` çağrılarının bağımlı olduğu `tables` state'i değiştiğinde zaten yeniden render olduğu için şu anki yapıda memoization aşırı olur; asıl iyileştirme, durum yönetimini `useReducer` ile birleştirip tek bir state güncellemesiyle birden fazla işlemi halletmek olabilir.
- showFeedback içindeki setTimeout'ı useRef + useEffect cleanup ile yöneterek, art arda tetiklemelerde stale state güncellemelerini ve olası bellek sızıntısını önleyin.
- getTableStyle hesabını ve Array.from çağrısını useMemo ile sararak her render'da gereksiz string birleştirme/dizi oluşturma maliyetini azaltın.

### Güvenlik — 98/100

**Öneriler:**
- Ödeme işlemi gerçek bir sunucuya bağlandığında HTTPS zorunlu olmalı ve sunucu tarafında yetkilendirme kontrolü yapılmalıdır.
- Gelecekte kullanıcıdan alınacak string girdiler (ör. masa notları) eklenirse, bunlar JSX'e doğrudan gömülmeli ve sanitizasyonsuz olarak dangerouslySetInnerHTML kullanılmamalıdır.
- Bu kapsamda XSS veya token yönetimi gerektirecek bir backend/AI entegrasyonu eklenirse, kullanıcı girdisini render etmeden önce DOMPurify ile sanitize edin ve JWT gibi token'ları httpOnly cookie'de saklayın.
- Ödeme modalı gibi hassas akışlarda ileride gerçek PCI/DSS kapsamına girilecekse, kart verilerini asla component state'inde tutmayın; Stripe Elements gibi tokenize edilmiş sağlayıcı kullanın.

### Sürdürülebilirlik — 44/100

**Öneriler:**
- Bileşeni küçük parçalara ayırın: TableNode, ControlBar, PaymentModal, SplitBillModal gibi alt bileşenler oluşturun.
- Tablo seçimi, birleştirme ve ödeme mantığını custom hook'lara (useTableSelection, useTableActions) taşıyarak sunum katmanından ayırın.
- RestaurantFloorPlan bileşenini <Table>, <FloorGrid>, <ActionBar>, <Modal> gibi alt bileşenlere ve useTableActions custom hook'una ayırarak sorumlulukları net şekilde ayırın.
- Payment ve Split Bill modallarını ortak bir <Modal> bileşenine (children/footer prop'ları ile) ve status string'lerini bir TypeScript enum'ı veya sabitler dosyasına çıkararak tekrarı ve magic string'leri ortadan kaldırın.
...
**Öneriler:**
- Merged gruplar için tablo seçiminde “Unmerge” butonu yerine, alt kontrol çubuğunda ayrı bir “Unmerge” aksiyonu eklenerek kullanıcı deneyimi iyileştirilebilir.
- Split Bill ve Payment işlemlerinde yalnızca ‘occupied’ masalar seçilebilir kısıtı net çalışıyor; ancak birden fazla ‘occupied’ masanın aynı anda ödeme/split işlemine sokulamaması isteniyorsa butonların disabled durumu buna göre ayarlanmalıdır.
- Prompt 'green' ve 'yellow' için tam tonlar istemişti; emerald ve amber tonları bunu karşılasa da, istenirse Tailwind 'green-500' ve 'yellow-400' ile daha net eşleşme sağlanabilir.
- Renkler ve durum eşlemesi için tek bir sabit (örn. STATUS_MAP) iyi olsa da, 'merged' durumu prompt'ta istenmediğinden çıkarılıp sadece üç renk (vacant/occupied/reserved) üzerine odaklanılabilir.
...
| Sürdürülebilirlik | 80 | 76 | — | **78** | Ortalama (J1-J2) |
...
**Öneriler:**
- Anlamı net olmayan '3' gibi sihirli sayıları açıklayıcı bir sabit (ör. MAX_TABLE_SELECTION) ile değiştirin.
- Modal bileşenindeki 'animate-in fade-in zoom-in' gibi Tailwind sınıflarını daha kısa ve tutarlı bir düzende yazın (ör. gruplandırma).
- Tekrarlayan buton ve kart stil sınıflarını (gradient butonlar, modal footer'lar) küçük helper bileşenlere (örn. GradientButton, ModalFooter) çıkararak hem satır uzunluğunu hem de DRY ihlalini azaltın.
- handleConfirmMerge ve handleConfirmPayment gibi karmaşık business logic fonksiyonlarının üstüne tek satırlık JSDoc yorumları (örn. 'Merges selected tables into a single occupied table, preserving aggregated seat/guest/total') ekleyin.

### Performans — 75/100

**Öneriler:**
- `showNotification` fonksiyonunu `useCallback` ile sarıp `handleSelectTable` daki dependency array’e ekleyerek doğru referans bağımlılığını sağlayın.
- Modal içindeki toplam hesaplama işlemlerini (reduce) `useMemo` ile sararak her render'da tekrar hesaplanmasını önleyin.
- `TableCard` bileşenini `React.memo` ile sarın ve `handleSelectTable`'ı zaten `useCallback` ile stabilize edildiği için sadece değişen kartlar render olsun.
- `showNotification` içindeki `setTimeout`'ı `useEffect` cleanup'ında veya bir ref ile tutarak component unmount olduğunda `clearTimeout` ile temizleyin; ayrıca `handleSplitBill`, `handleMergeTables`, `handleProcessPayment` gibi fonksiyonları `useCallback` ile sarın.

### Güvenlik — 95/100

**Öneriler:**
- Üretimde kullanılacaksa, gerçek API'lerle iletişimde HTTPS zorunlu kılınmalı ve hassas veriler localStorage yerine güvenli cookie/sunucu tarafı yönetilmelidir.
- Ödemeler gibi hassas işlemler için PCI-DSS uyumluluğu ve güçlü giriş doğrulaması eklenmelidir (bu demo için gerekli değil).
- İleride tablo adı, rezervasyon notu gibi kullanıcı girdileri eklenirse, render öncesi input validation ve sanitization uygulanmalıdır.
- Üretim ortamında birden fazla kullanıcı (garson, yönetici) olacaksa, role-based access control ve client-side route guard'lar eklenmelidir.

### Sürdürülebilirlik — 78/100

**Öneriler:**
- Tablo seçimi, fatura bölme gibi iş mantığını `useTableSelection` ve `useTableActions` gibi custom hook'lara taşıyarak bileşenin test edilebilirliğini ve yeniden kullanılabilirliğini artırın.
- İstatistik çubuğu ve alt kontrol çubuğunu ayrı bileşenlere (ör. `StatsBar`, `BottomActionBar`) ayırarak ana bileşenin sorumluluğunu azaltın.
- Masabaşı seçim mantığını (max 3 seçim, doğrulama, bildirim) ve istatistik hesaplamasını useFloorPlan ve useTableSelection gibi custom hook'lara çıkararak ana bileşeni sadeleştirin.
- SplitBill, MergeTables ve Payment modal içeriklerini ayrı bileşenlere (SplitBillModal, MergeTablesModal, PaymentModal) ayırarak her birini bağımsız test edilebilir hale getirin.
...
**Öneriler:**
- Maintenance durumu (gray) gibi ekstra renk kodlaması eklenmiş, ancak promptta sadece üç durum belirtilmiş; istenirse maintenance durumu kaldırılabilir.
- Promptta 'Merge Tables' ve 'Split Bill' işlevlerinin implementasyonu istenmemiş, fakat bunların eklenmesi kullanıcı deneyimini iyileştirir ve bir zorunluluk ihlali sayılmaz.
- Masa kartlarında durum renklerini (STATUS_CONFIG.color) ana vurgu rengi olarak yansıtmak, şu an yalnızca küçük top dot için kullanılıyor; kartın arka planında daha baskın olabilir.
- Split Bill modalındaki 'Equal Split', 'By Items' gibi seçenekler tıklanabilir görünüyor ancak seçim state'i yok; seçilen yöntemi görsel olarak vurgulamak için aktif durum yönetimi eklenebilir.
...
| Metrik | J1 | J2 | J3 | Final | Çözüm Yöntemi |
|---|---|---|---|---|---|
| Okunabilirlik | 68 | 78 | — | **73** | Ortalama (J1-J2) |
| Performans | 78 | 72 | — | **75** | Ortalama (J1-J2) |
| Güvenlik | 100 | 88 | — | **94** | Ortalama (J1-J2) |
| Sürdürülebilirlik | 55 | 38 | — | **47** | Ortalama (J1-J2) |
| Prompt Uyumu | 100 | 98 | — | **99** | Ortalama (J1-J2) |
...
**Öneriler:**
- Bileşeni FloorMap, TableCard, SidePanel, SplitModal, PaymentModal gibi alt bileşenlere bölerek yapısal netliği artırın.
- Kullanılmayan `useEffect` importunu ve `Icons.Plus` bileşenini kaldırarak kod temizliğini sağlayın.
- Kullanılmayan `useEffect` import'unu kaldırın; vergi oranı gibi magic number'ları dosya başında `TAX_RATE`/`SUBTOTAL_RATIO` sabitleri olarak tanımlayın.
- Bileşeni `FloorMap`, `TableCard`, `PaymentModal`, `SplitBillModal`, `ControlBar` gibi alt bileşenlere ayırarak dosya uzunluğunu azaltın ve sorumlulukları netleştirin.

### Performans — 75/100

**Öneriler:**
- `finalizePayment` içindeki `setTimeout` sonucunu bir ref'te saklayıp `useEffect` cleanup'ında `clearTimeout` ile temizleyerek bellek sızıntısını önleyin.
- Tablo stilleri için inline `statusClasses` hesaplamasını `useMemo` veya önceden tanımlanmış bir obje ile memoize ederek her render'da aynı string oluşturmayı engelleyin.
- finalizePayment içindeki setTimeout bir ref içinde saklanmalı ve bileşen unmount olduğunda veya modal kapandığında clearTimeout ile temizlenmelidir; böylece unmount sonrası state güncellemesi ve olası bellek sızıntısı önlenir.
- stats hesaplaması tek bir reduce döngüsünde birleştirilmeli; ayrıca tablo stilleri ve inline style objeleri useMemo/useCallback ile sarılarak her render'da gereksiz referans değişimleri engellenmelidir.

### Güvenlik — 94/100

**Öneriler:**
- Gerçek bir ödeme formu entegre edilecekse HTTPS kullanımı ve kredi kartı bilgilerinin güvenli işlenmesi sağlanmalı.
- Kullanıcıdan veri alınacaksa XSS koruması için DOMPurify gibi sanitizasyon kütüphaneleri kullanılmalı.
- `alert()` kullanımı XSS riski taşımasa da, bilgi sızıntısı potansiyeli nedeniyle özel bir toast/modal bileşeniyle değiştirilmelidir.
- Gelecekte gerçek kullanıcı girdisi veya API entegrasyonu eklenecekse, token'ları HttpOnly cookie'de saklayın ve localStorage'a yazmaktan kaçının; ayrıca tüm gelen veriyi render etmeden önce DOMPurify ile sanitize edin.

### Sürdürülebilirlik — 47/100

**Öneriler:**
- Bileşeni SRP'ye uygun hale getirin: özel bir custom hook (ör. useFloorPlan) ile state ve iş mantığını ayrıştırın; en az 3-4 alt bileşen (SectionTabs, TableGrid, TableCard, BottomBar) oluşturarak sunumu modülerleştirin.
- Sertçe kodlanmış veri (INITIAL_TABLES) ve sabitleri (durum renkleri, label'lar) ayrı bir config.ts veya constants.ts dosyasına çıkarın; null/undefined kontrolleri için optional chaining veya null-koalesans kullanarak savunmacı programlamayı güçlendirin.
- Bileşeni TableCard, StatsCounter, SectionTabs, TableDetailPanel, BottomControlBar, SplitBillModal, PaymentModal gibi alt bileşenlere ayırarak SRP'yi uygulayın ve props ile veri akışını netleştirin.
- Status ve modal türleri için constants.ts içinde enum/sabit diziler tanımlayın; merge ve ödeme gibi iş mantığını custom hook'lara (useTableActions, usePayment) çıkararak saf fonksiyonlar hâline getirin.
...
**Öneriler:**
- Kontrol çubuğundaki butonların işlevsellikleri (örneğin Split Bill) daha detaylı alt akışlarla zenginleştirilebilir.
- Tablo seçim modunun görsel geri bildirimi iyileştirilebilir, 'Merge Tables' modunda seçim sayısına göre daha belirgin ipuçları eklenebilir.
- Split by Item seçeneği için ayrı bir gerçek UI akışı eklenip alert yerine inline atama arayüzü sunulabilir.
- Merge işlemi sonrası birleştirilen masaların orijinal hücrelerini gerçekten silmek yerine sadece durumlarını boşaltıyor; tek bir 'composite' kayıt modeli kullanmak veri tutarlılığını artırır.
