...
|---|---|---|---|---|---|---|
| DeepSeek V4 Pro | 81 | 79 | 84 | 96 | 47 | 98 |
| Qwen3.6 Plus | 86 | 79 | 78 | 93 | 79 | 99 |
| Gemini 3.5 Flash | 77 | 71 | 87 | 95 | 32 | 98 |
...
|---|---|---|---|---|---|
| Okunabilirlik | 82 | 75 | — | **79** | Ortalama (J1-J2) |
| Performans | 90 | 78 | — | **84** | Ortalama (J1-J2) |
| Güvenlik | 100 | 92 | — | **96** | Ortalama (J1-J2) |
| Sürdürülebilirlik | 38 | 55 | — | **47** | Ortalama (J1-J2) |
...
**Öneriler:**
- 194. satırdaki uzun inline stilleri bir helper fonksiyona taşıyarak veya CSS değişkenleri kullanarak JSX içindeki karmaşayı azaltın.
- iconMap ve colors gibi durumlarda her seferinde yeniden hesaplama yapmak yerine, map içerisinde destructuring kullanarak kodu sadeleştirin.
- Remove unused `useCallback` import and extract `useCallback` wrappers if needed, or drop the import entirely.
- Extract a SHAPE_DIMENSIONS constant object (e.g., { circle: { w: 80, h: 80 }, square: { w: 96, h: 96 } }) to replace the magic numbers in inline style calculations and the shape-class mapping.

### Performans — 84/100

**Öneriler:**
- `getTableStyle` fonksiyonu her render'da yeniden oluşturuluyor; performansı artırmak için `useCallback` ile sarılabilir.
- `tables.map` içindeki `STATUS_MAP[table.status]` erişimi her render'da tekrarlanıyor; `useMemo` ile tablo stilleri önceden hesaplanabilir.
- showFeedback içindeki setTimeout'u useEffect cleanup veya AbortController ile yöneterek unmount sonrası setState uyarılarını önleyin.
- Her render'da yeniden oluşan inline style nesneleri ve getTableStyle string'leri için useMemo/useCallback ile küçük bir memoization katmanı ekleyin (veya TableCard'ı React.memo ile sarın).

### Güvenlik — 96/100

**Öneriler:**
- İleride tablo verileri bir API'den alınırsa, gelen veriyi render etmeden önce XSS'e karşı DOMPurify gibi bir sanitizasyon kütüphanesi kullanılmalıdır.
- Kullanıcıdan metin girişi eklenecekse (ör. rezervasyon notları), otomatik React koruması yeterli olmakla birlikte ek doğrulama ve uzunluk sınırlaması yapılması önerilir.
- Modallar ve toast mesajları kullanıcı girdisi taşıyacaksa (örn. dinamik fiyat, müşteri adı) DOMPurify veya kaçış (escaping) ile sanitize edilmelidir.
- Gelecekte API entegrasyonu eklendiğinde hassas veriler (token, kart bilgisi) yalnızca HttpOnly cookie veya güvenli backend üzerinden yönetilmeli, console'a yazılmamalıdır.

### Sürdürülebilirlik — 47/100

**Öneriler:**
- Bileşeni TableGrid, ControlBar, TableNode, PaymentModal vb. alt bileşenlere ayırın; durum yönetimi için özel bir hook (useTableManagement) oluşturun.
- Sabitleri (INITIAL_TABLES, STATUS_MAP, GRID_COLS/ROWS) ayrı bir config/constants dosyasına taşıyın ve şekil-sınıf eşleşmeleri için bir obje haritası kullanarak magic string’leri azaltın.
- PaymentModal ve SplitBillModal'u ortak bir yeniden kullanılabilir Modal bileşenine ve tablo render mantığını TableCard + FloorPlanGrid alt bileşenlerine ayırarak monolitik yapıyı çözün.
- Tüm state ve aksiyon mantığını (toggleTableSelection, handleMerge, finalizePayment vb.) useFloorPlan adlı özel bir hook'a taşıyarak sunum katmanından ayırın ve magic string'leri bir constants/enum dosyasında toplayın.
...
**Öneriler:**
- Tablo birleştirme işlemi sonrası gruplandırmayı vurgulamak için daha belirgin görsel ipuçları eklenebilir.
- Split Bill ve Payment modallarında işlem sonrası başarı animasyonu eklenerek kullanıcı deneyimi iyileştirilebilir.
- Prompt'ta 'grid' denseği vurgulanmış; absolute positioning yerine native CSS grid ile tabloları doğrudan grid hücrelerine yerleştirmek yerleşim sadakatini daha net yansıtır.
- Status renkleri için doğrudan emerald-500/rose-500/amber-500 gibi solid badge renklerini tablo arka planına da yansıtarak renk kodlamasını daha görünür kılabilirsin.
...
|---|---|---|---|---|---|
| Okunabilirlik | 82 | 75 | — | **79** | Ortalama (J1-J2) |
| Performans | 78 | 78 | — | **78** | Ortalama (J1-J2) |
| Güvenlik | 98 | 88 | — | **93** | Ortalama (J1-J2) |
| Sürdürülebilirlik | 82 | 75 | — | **79** | Ortalama (J1-J2) |
...
**Öneriler:**
- Sihirli sayıları (ör. maksimum seçim sayısı 3, bildirim süresi 3000) anlamlı sabitlerle tanımlayın: `MAX_SELECTABLE_TABLES = 3`, `NOTIFICATION_DURATION_MS = 3000`.
- Karmaşık iş mantığı içeren `handleConfirmMerge` ve `handleConfirmPayment` gibi fonksiyonlara kısa açıklayıcı yorumlar ekleyerek neden bu şekilde yapıldığını belirtin.
- Ana bileşenden modal içeriklerini (SplitBillModal, MergeTablesModal, PaymentModal) ayrı bileşenlere çıkararak RestaurantFloorPlan'ı ~150 satıra düşürün.
- Bottom bar ve modal butonları için ortak bir Button bileşeni (variant: 'primary' | 'secondary' | 'success' | 'danger' gibi) tanımlayarak tekrar eden Tailwind class yığınlarını ortadan kaldırın.

### Performans — 78/100

**Öneriler:**
- handleSelectTable useCallback bağımlılık dizisine showNotification eklenmeli veya showNotification da useCallback ile sarılmalı; aksi halde stale closure oluşabilir.
- Modal içindeki buton onClick'leri useCallback ile memoize edilmeli veya Modal bileşeni React.memo ile sarılarak gereksiz yeniden render'lar önlenmeli.
- showNotification içindeki setTimeout'ı useRef + clearTimeout ile temizleyin; bileşen unmount olduğunda 'setState on unmounted component' hatası ve bellek sızıntısı riskini ortadan kaldırın.
- TableCard bileşenini React.memo ile sarın ve handleSplitBill/handleMergeTables/handleProcessPayment gibi callback'leri useCallback ile memoize ederek gereksiz re-render'ları önleyin.

### Güvenlik — 93/100

**Öneriler:**
- Eğer gelecekte dış kaynaklardan (API) gelen veriler dinamik olarak render edilecekse, DOMPurify gibi bir sanitizasyon kütüphanesi kullanılması önerilir.
- Projeye gizli anahtarlar eklenirse, bunların .env dosyasında uygun prefix ile saklandığından ve kod içine gömülmediğinden emin olunmalıdır.
- Gelecekte query string (ör. ?tableId=) veya form alanları eklenecekse URLSearchParams ve whitelist tabanlı input validation uygulanmalıdır.
- Ödeme işlemleri için gerçek backend entegrasyonu yapıldığında HTTPS zorunluluğu, CSP header'ları ve hassas kart verisi için PCI-DSS uyumlu tokenizasyon (ör. Stripe Elements) kullanılmalıdır.

### Sürdürülebilirlik — 79/100

**Öneriler:**
- Ana bileşendeki iş mantığını (tablo seçimi, ödeme işlemleri vb.) özel hook'lara (useTableSelection, useTableActions) taşıyarak test edilebilirliği ve modülerliği artırın.
- Modal türleri için 'splitBill', 'mergeTables' gibi string değerleri sabit bir enum veya nesne içinde tanımlayarak sihirli string kullanımını azaltın.
- Modal içeriklerini (SplitBillModal, MergeTablesModal, PaymentModal) ve business logic'lerini (handleConfirm*, stats) ayrı bileşenlere ve özel hook'lara (useTableActions, useTableStats) çıkararak ana bileşeni sadeleştirin.
- TypeScript veya en azından PropTypes kullanarak bileşen sözleşmelerini belgelendirin; ayrıca ortak Tailwind class kombinasyonlarını (örn. `flex-1 px-4 py-3 bg-gray-100 text-gray-700 rounded-xl`) cn() yardımcısı ile birleştirerek tekrarları azaltın.
...
**Öneriler:**
- Renk kodlamasında istenen doğrudan yeşil/kırmızı/sarı değil ama benzer anlamlar var; tam olarak 'green/red/yellow' isimli renkler kullanılmasa da semantik karşılanmış.
- Şu haliyle prompta tam uyumlu, geliştirme önerisi olarak masa kartlarında animasyon veya hover efektleri biraz daha sadeleştirilebilir.
- Split Bill modalda seçili masanın 'guests' sayısına göre otomatik 'Equal Split' hesaplaması gösterilebilir, kullanıcıya daha somut bir önizleme sunulur.
- Rezervasyon saatlerine ek olarak rezervasyon sahibi adı veya müşteri bilgisi gibi alanlar eklenerek içerik zenginleştirilebilir.
...
| Metrik | J1 | J2 | J3 | Final | Çözüm Yöntemi |
|---|---|---|---|---|---|
| Okunabilirlik | 70 | 72 | — | **71** | Ortalama (J1-J2) |
...
### Okunabilirlik — 71/100

**Öneriler:**
- INITIAL_TABLES ve Icons nesnelerini ayrı dosyalara (ör. mockData.ts, icons.tsx) taşıyarak ana bileşen dosyasını 150 satır altına indirin. Bu, bileşenin temel mantığının daha kolay okunmasını sağlar.
- JSX içindeki koşullu stil mantığını (statusClasses, glowColor gibi) ayrı bir yardımcı fonksiyona çıkarın. Bu, render bloğundaki görsel gürültüyü azaltır.
- Extract modals (SplitBill, ProcessPayment) and the Icons object into separate files/components to reduce main component length below 300 lines.
- Remove the unused useEffect import and extract magic numbers (0.85 subtotal factor, 1500ms timeout) into named constants like SUBTOTAL_RATIO and PAYMENT_FEEDBACK_DELAY_MS.

### Performans — 87/100

**Öneriler:**
- Büyük tablo listelerinde performans için tablo butonlarını React.memo ile sarmalayın ve onClick callback'lerini useCallback ile memoize edin.
- setTimeout callback'ini bir useEffect içinde yöneterek, bileşen kapanmadan önce clearTimeout ile iptal edin.
- finalizePayment içindeki setTimeout'ı useEffect + clearTimeout ile ya da AbortController benzeri bir mekanizma ile cleanup ederek olası state-update-on-unmounted-component ve bellek sızıntısı riskini ortadan kaldırın.
- executeMerge ve stats hesaplamasında çoklu tables.find/filter çağrılarını tek bir Map oluşturup O(1) lookup ile ya da tek bir reduce pass'ı ile birleştirerek gereksiz iterasyonları azaltın.

### Güvenlik — 95/100

**Öneriler:**
- Gerçek bir API entegrasyonunda, kullanıcıdan alınacak her türlü veri (masa kapasitesi, isim vb.) client-side olarak doğrulanmalı ve HTML render eden yapılar (`dangerouslySetInnerHTML`) kullanılıyorsa DOMPurify gibi bir sanitizer ile temizlenmelidir.
- Ödeme ve işlem akışları produksiyona alınmadan önce HTTPS zorunlu hale getirilmeli ve hassas veriler asla state'te açık metin olarak uzun süre tutulmamalıdır.
- Masa adı veya sunucu adı gibi alanlar gelecekte kullanıcı girdisi alacaksa, render öncesi HTML entity sanitization ekleyin.
- Geriçek ödeme entegrasyonu eklendiğinde, kart bilgisi gibi hassas verileri state'de tutmak yerine tokenizasyon (PCI-DSS uyumlu sağlayıcı) kullanın.

### Sürdürülebilirlik — 32/100

**Öneriler:**
- Bileşeni küçük, sorumluluk sahibi alt bileşenlere (TableGrid, TableCard, BottomBar, PaymentModal vb.) ayırın ve iş mantığını custom hook'lara (useTableManagement, usePayment) taşıyın.
- Sabit değerleri (renkler, durum metinleri, konfigürasyon) ayrı bir constants dosyasına çıkarın ve stil sınıflarını dinamik olarak hesaplamak için yardımcı fonksiyonlar kullanın.
- RestaurantFloorPlan bileşenini TableCard, StatsCounter, SectionTabs, DetailPanel, BottomControlBar, SplitBillModal ve PaymentModal gibi küçük, tek sorumluluklu alt bileşenlere ayırın.
- Status sabitlerini (TABLE_STATUS, SHAPE_TYPES) ayrı bir constants.ts dosyasına çıkarın; useTableSelection, useMergeTables, usePayment gibi custom hook'larla iş mantığını sunumdan ayırın.
...
**Öneriler:**
- Kod, tüm gereksinimleri karşıladığı için iyileştirme önermeye gerek yoktur.
- Gelecekteki geliştirmelerde koyu mod gibi özellikler eklenebilir ancak mevcut haliyle talimatlara tam uyumludur.
- Add a visual representation of actual table positions (x,y coordinates) on a canvas-style floor plan to better match 'floor plan' semantics
- Include keyboard shortcuts or drag-and-drop for table status changes to enhance the 'dynamic' requirement
