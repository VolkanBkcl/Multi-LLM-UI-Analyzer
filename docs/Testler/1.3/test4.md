...
|---|---|---|---|---|---|---|
| DeepSeek V4 Pro | 77 | 83 | 78 | 83 | 40 | 99 |
| Qwen3.6 Plus | 76 | 75 | 74 | 91 | 39 | 99 |
| Gemini 3.5 Flash | 76 | 62 | 81 | 95 | 44 | 98 |
...
|---|---|---|---|---|---|
| Okunabilirlik | 85 | 80 | — | **83** | Ortalama (J1-J2) |
| Performans | 75 | 80 | — | **78** | Ortalama (J1-J2) |
| Güvenlik | 75 | 90 | — | **83** | Ortalama (J1-J2) |
| Sürdürülebilirlik | 40 | 40 | — | **40** | Ortalama (J1-J2) |
...
**Öneriler:**
- Büyük bileşenlerin okunabilirliğini artırmak için, Dashboard bileşenini daha küçük alt bileşenlere (ör. TopNav, StreakWidget, CourseCard gibi) bölmek faydalı olacaktır.
- Boolean değişkenlerin (ör. urgent) başına 'is' veya 'has' gibi açıklayıcı önekler ekleyerek anlamı netleştirebilirsiniz.
- Dashboard bileşenini Navbar, StreakWidget, CourseCard ve AssignmentList gibi alt bileşenlere ayırarak dosya uzunluğunu azaltın ve her bölümün sorumluluğunu netleştirin.
- Açıklayıcı olmayan '// Sample data' gibi yorumları kaldırın; bunun yerine karmaşık iş mantığı (ör. circular progress hesaplaması) için neden yapıldığını açıklayan kısa notlar ekleyin.

### Performans — 78/100

**Öneriler:**
- `new Date(assignment.due).toLocaleDateString(...)` çağrısı render içinde tekrarlı yapılıyor; bu tarih formatlaması `useMemo` ile veya veri katmanında önceden biçimlendirilerek tek seferlik hesaplanabilir.
- .animate-appear animasyonu `<style jsx>` bloğuyla global tanımlanmış; CSS modülü veya Tailwind eklentisi kullanılmadığı takdirde bu stil her render'da yeniden oluşturulabilir. Stili ayrı bir CSS dosyasına taşıyıp import ederek optimize edin.
- Sabit veri setlerini (courses, assignments, user) bileşen dışına taşıyın veya useMemo ile sarın; ayrıca tarih formatlamasını map dışında önceden hesaplayın.
- Dropdown toggle handler'ını ve dış tıklama dinleyicisini useCallback ile sararak gereksiz re-render'ları önleyin.

### Güvenlik — 83/100

**Öneriler:**
- Kullanıcı girdisi veya dış API'lerden gelen veriler (isim, e-posta vb.) render edilmeden önce HTML entity escape veya DOMPurify ile sanitize edilmeli.
- Harici görsel URL'leri için src doğrulaması (URL.protocol kontrolü) eklenerek yalnızca https:// kaynaklarına izin verilmeli.
- Görsel src'lerinde 'güvenli' kaynaklar kullanılsa da, kullanıcı tarafından sağlanabilecek dinamik URL'lerde Content Security Policy (CSP) ve rel='noopener noreferrer' gibi ek önlemler değerlendirilmeli.
- Gelecekte form/input alanları eklenecekse, XSS ve injection'a karşı DOMPurify ile sanitization ve tüm kullanıcı girdileri için server-side doğrulama planlanmalı.

### Sürdürülebilirlik — 40/100

**Öneriler:**
- Dashboard bileşenini Navbar, ProfileDropdown, StreakWidget, CourseCard ve AssignmentList gibi bağımsız alt bileşenlere ayırın.
- Veri dizilerini (courses, assignments, user) ayrı bir constants.js dosyasına taşıyın ve dropdown dış tıklama mantığını useClickOutside gibi özel bir hook'a çıkarın.
- Bileşeni NavBar, StreakWidget, CourseCard, AssignmentItem, AssignmentSidebar gibi alt bileşenlere ayırın; streak hesaplama ve dış-tıklama mantığını useClickOutside ve useStreakProgress gibi custom hook'lara taşıyın.
- courses, assignments, user ve streak verilerini src/data/ altında ayrı dosyalara, ortak renk/etiket sabitlerini de constants.ts dosyasına çıkararak modifiability ve testability'yi artırın.
...
**Öneriler:**
- Tüm semantik gereksinimler karşılanmıştır, harika iş çıkardınız.
- Replace the `<style jsx>` block (a Next.js-specific feature) with either a regular `<style>` tag in `useEffect` or a Tailwind `animate-*` utility to ensure the dropdown animation works in a plain React (CRA/Vite) environment.
- Consider adding aria-expanded and role='menu' attributes to the profile dropdown for stronger accessibility alignment with the 'production-ready' system constraint.
...
| Metrik | J1 | J2 | J3 | Final | Çözüm Yöntemi |
|---|---|---|---|---|---|
| Okunabilirlik | 75 | 74 | — | **75** | Ortalama (J1-J2) |
| Performans | 65 | 82 | — | **74** | Ortalama (J1-J2) |
| Güvenlik | 90 | 92 | — | **91** | Ortalama (J1-J2) |
...
**Öneriler:**
- Assignment listesindeki map içinde kullanılmayan 'index' değişkenini kaldırın veya key olarak kullanın. Ayrıca 'bg-white/80' gibi geçersiz Tailwind opacity değerlerini düzeltin.
- Donanımsal değerleri (ör. 3000, 100) anlamlı sabitlere taşıyın ve 'px-4 py-2.5' gibi standart olmayan padding değerlerini tutarlı hale getirin.
- StudentDashboard'ı Navbar, StreakWidget, CourseCard, AssignmentsSidebar gibi alt bileşenlere ayırarak dosya uzunluğunu düşürün ve her parçayı bağımsız olarak okunabilir hale getirin.
- Kurs, assignment, badge ve priority/status için üst seviyede sabitler (örn. PRIORITY_COLORS, STATUS_ICONS) tanımlayıp helper fonksiyonları saf tutarak veri ile sunumu net biçimde ayırın.

### Performans — 74/100

**Öneriler:**
- Statik course ve assignment dizileri bileşen dışına çıkarılmalı veya mutate edilmiyorsa useMemo ile sarılmalı.
- map içinde index'in key olarak kullanımı performans sorununa yol açabilir, benzersiz id kullanılmalı (badge listesi).
- getPriorityColor ve getStatusIcon gibi saf fonksiyonlar bileşen dışına alınıp memoize edilmeli veya her render'da yeniden tanımlanmamaları sağlanmalıdır.
- user, streakData, courses, assignments veri objelerini component dışına (modül seviyesine) taşıyarak her render'da yeniden oluşturulmalarını önleyin; bu sayede referans eşitliği de korunur.
- getPriorityColor ve getStatusIcon fonksiyonlarını component dışına çıkararak veya useCallback ile sararak her render'da yeniden tahsis edilmelerini engelleyebilirsiniz.

### Güvenlik — 91/100

**Öneriler:**
- Kullanıcı verileri (isim, e-posta) bir API'den alınacaksa, bu verilerin güvenli bir şekilde state'e aktarıldığından ve doğrudan koda gömülmediğinden emin olun. Statik veriler demo amaçlı ise bu bir sorun değildir, ancak production'da hassas veriler kod içinde yer almamalıdır.
- Gelecekte eklenecek form alanları veya URL parametreleri için client-side input doğrulaması (e-posta formatı, maksimum uzunluk vb.) eklenmesi önerilir. Bu, dashboard üzerinde kullanıcı etkileşimi arttığında güvenliği sağlayacaktır.
- Dropdown dışı tıklama için `mousedown` yerine Escape tuşu desteği ve klavye erişilebilirliği (ARIA) ekleyerek kullanıcı etkileşim güvenliğini artırın.
- Gelecekte API entegrasyonu yapılacaksa HTTP isteklerini HTTPS'e zorlayan bir base URL yapılandırması ve response içeriklerini sanitize etme altyapısını baştan kurun.

### Sürdürülebilirlik — 39/100

**Öneriler:**
- Bileşeni Navbar, StreakWidget, CourseCard, AssignmentList gibi küçük, yeniden kullanılabilir alt bileşenlere ayırın.
- Veri ve yardımcı fonksiyonları ayrı dosyalara (constants.ts, utils.ts) çıkarın; state yönetimi için custom hook'lar kullanın.
- Parçaları ayrı bileşenlere bölün: `<NavBar>`, `<StreakWidget>`, `<CourseCard>`, `<AssignmentItem>`, `<UserDropdown>` ve `<ProgressBar>` gibi ortak bileşenleri ayrı dosyalara çıkararak SRP'yi uygulayın.
- Veri ve sabitleri dışarı taşıyın: `user`, `courses`, `assignments`, `streakData` verilerini `data/` klasörüne, `priority` ve `status` enum'larını `constants.ts` dosyasına taşıyın; ayrıca `useClickOutside` gibi mantığı özel bir hook'a (`hooks/useClickOutside.ts`) çıkarın.
...
**Öneriler:**
- Eksiksiz uygulanmış, iyileştirme önerisi yok.
- Sidebar'a 'Filter' veya 'Sort' (priority/deadline) gibi etkileşimli kontroller eklenebilir.
- Dropdown menüye klavye erişilebilirliği (Escape ile kapatma, Tab navigasyonu) eklenebilir.
...
| Metrik | J1 | J2 | J3 | Final | Çözüm Yöntemi |
|---|---|---|---|---|---|
| Okunabilirlik | 65 | 58 | — | **62** | Ortalama (J1-J2) |
| Performans | 90 | 72 | — | **81** | Ortalama (J1-J2) |
| Güvenlik | 100 | 90 | — | **95** | Ortalama (J1-J2) |
| Sürdürülebilirlik | 40 | 48 | — | **44** | Ortalama (J1-J2) |
| Prompt Uyumu | 100 | 95 | — | **98** | Ortalama (J1-J2) |
...
**Öneriler:**
- Bileşeni daha küçük alt bileşenlere ayırın (ör. TopNav, GamificationWidget, CourseCard, DeadlinesSidebar).
- Kullanılmayan 'BookOpen' import'unu kaldırarak kod temizliğini iyileştirin.
- Extract reusable subcomponents (StreakWidget, CourseCard, DeadlineItem, ProfileDropdown, NotificationsDropdown) to bring the main component under 150 lines and improve scannability.
- Replace non-standard Tailwind classes (w-5.5, h-5.5, text-indigo-150, border-slate-150) with valid utilities (e.g., w-5 h-5, text-indigo-200, border-slate-200) and remove the unused `BookOpen` import to keep the code production-ready and unambiguous.

### Performans — 81/100

**Öneriler:**
- Notification ve profil dropdown menüleri her render'da yeniden oluşturuluyor. Bu durum basit statik içerik için sorun olmasa da, ileride bu bileşenler karmaşıklaşırsa React.memo ile korunabilir.
- Inline stil hesaplamaları (width hesaplamaları gibi) render sırasında yapılıyor. Bu hesaplamalar çok basit olsa da, ileride data seti büyüdüğünde tutarlılık açısından useMemo ile sarılabilir.
- Büyük bileşeni ProfileDropdown, NotificationDropdown, CourseCard, DeadlineItem, StreakWidget gibi alt bileşenlere ayırın ve React.memo ile sarın; böylece dropdown veya arama state değişimlerinde sadece ilgili bölümler render olur.
- setIsProfileDropdownOpen ve setIsNotificationsOpen gibi inline callback'leri useCallback ile memoize edin; ayrıca width gibi inline style objelerini useMemo ile ya da sabit değişkenlerle optimize edin.

### Güvenlik — 95/100

**Öneriler:**
- Gerçek API verisi ile entegrasyon sağlandığında, kullanıcı girdisi veya API yanıtı render edilmeden önce XSS’e karşı sanitizasyon uygulanmalıdır. Özellikle HTML içerik render edilecekse DOMPurify gibi bir kütüphane kullanmayı planlayın.
- Uygulamaya kimlik doğrulama eklenirse, JWT veya tokenları localStorage yerine httpOnly cookie üzerinde saklamayı tercih edin.
- Arama girişine maxLength ve basit sanitization ekleyerek aşırı uzun veya zararlı girdileri sınırlandırın.
- Gelecekte API entegrasyonu eklenecekse HTTPS zorunluluğu, token'ların HttpOnly cookie veya secure storage'da tutulması ve input validation kuralları baştan tasarlanmalıdır.

### Sürdürülebilirlik — 44/100

**Öneriler:**
- Büyük bileşeni TopNav, CourseCard, DeadlineItem, GamificationWidget gibi küçük, yeniden kullanılabilir bileşenlere ayırın.
- Dropdown mantığını useClickOutside gibi bir custom hook'a taşıyın; urgency gibi sabit değerleri enum veya sabit dosyalara çıkarın.
- StreakWidget, CourseCard, DeadlineItem, ProfileDropdown, NotificationsDropdown, ProgressBar gibi alt bileşenleri ayrı dosyalara çıkararak SRP'yi uygulayın ve test edilebilirliği artırın.
- Urgency seviyeleri ve bildirim tipleri için TypeScript enum veya sabit nesneler (ör. URGENCY_LEVELS) tanımlayıp magic string'leri ortadan kaldırın; useClickOutside ve useToggle gibi custom hook'lar oluşturun.
...
**Öneriler:**
- Arama çubuğu işlevsel olarak implement edilmiş ancak 'Search courses...' placeholder'ı görünüyor; prompt'ta belirtilmemiş olsa da üretime hazır kod için arama sonuçlarını filtreleyecek bir state yönetimi eksik. Bunu useEffect ile mock veriler üzerinde filtreleme ekleyerek tamamlayabilirsiniz.
- Bildirim dropdown'ında 'Mark all read' butonu işlevsellikten yoksun; kullanıcı etkileşimini tamamlamak için onClick handler ve bildirim durumunu yönetecek bir state eklenmesi gerekir.
- Non-standard Tailwind utility sınıflarını (örn. w-5.5, border-slate-150, text-indigo-150) standart sınıflarla (w-5/w-6, border-slate-200, text-indigo-200) veya arbitrary value sözdizimi (w-[22px]) ile değiştirin.
- Sidebar'a 'filter' veya 'sort' gibi ek etkileşimler ekleyerek (örn. aciliyet sıralaması toggle'ı) prompt'taki 'sidebar' gereksinimini daha belirgin hale getirin.
