...
|---|---|---|---|---|---|---|
| DeepSeek V4 Pro | 80 | 84 | 85 | 94 | 37 | 98 |
| Qwen3.6 Plus | 80 | 84 | 83 | 98 | 39 | 98 |
| Gemini 3.5 Flash | 78 | 79 | 84 | 90 | 40 | 99 |
...
| Performans | 88 | 82 | — | **85** | Ortalama (J1-J2) |
...
**Öneriler:**
- Bileşen uzunluğu 250 satır civarında; daha küçük alt bileşenlere (ör. CourseCard, AssignmentItem) ayırarak okunabilirliği artırabilirsiniz.
- Boolean state değişkeni 'profileOpen' için 'isProfileOpen' gibi 'is' öneki kullanmak, boolean olduğunu daha açık hale getirir.
- Bileşeni daha küçük parçalara ayırın (ör. Navbar, StreakWidget, CourseCard, AssignmentSidebar) — her biri tek sorumluluk taşısın ve ana dosya okumayı kolaylaştırsın.
- 'Next Milestone' ve 'Tomorrow' gibi değerleri veriden türetin (ör. `Math.max(0, streak.max - streak.current)`); bu hem okunabilirliği artırır hem de veri değiştiğinde yanlışlıkları önler.

### Performans — 85/100

**Öneriler:**
- Sabit veri dizileri (courses, assignments, user) bileşen dışında tanımlanarak her render'da yeniden oluşturulması önlenebilir veya useMemo ile sarılabilir.
- Kurs kartlarında kullanılan 'Continue Learning' butonuna herhangi bir işlev eklenmemiş; gerçek uygulamada her render'da yeniden oluşturulan inline arrow fonksiyonlar yerine useCallback kullanımı düşünülebilir.
- Dropdown `profileOpen` true olduğunda event listener'ı ekleyip, false olduğunda kaldırarak gereksiz global listener yükünü azaltın.
- Statik verileri (user, courses, assignments) component dışında module-level sabitler olarak tanımlayarak her render'da yeniden allocation'ı önleyin.

### Güvenlik — 94/100

**Öneriler:**
- Kullanıcı avatarı gibi dış kaynaklardan alınan statik URL'lerin olası protokol karışıklığını önlemek için HTTPS zorunluluğunu kontrol edecek bir yardımcı fonksiyon eklenebilir.
- Email gibi statik veriler doğrudan ekrana yazdırılırken bile, ileride API'den gelen verilerle genişletileceği düşünülerek output encoding prensibi hatırlatılmalıdır.
- Replace placeholder `href="#"` anchors with proper routes or `button` elements to prevent unintended page jumps and improve accessibility.
- If future versions include user data or inputs, sanitize any HTML rendering with DOMPurify and validate URL parameters before use.

### Sürdürülebilirlik — 37/100

**Öneriler:**
- Bileşenleri küçük, bağımsız parçalara ayırın (Navbar, CourseCard, AssignmentList, StreakWidget).
- Veri ve yapılandırmayı ayrı dosyalara çıkarın ve custom hook'lar ile mantığı soyutlayın.
- Dashboard'ı NavBar, ProfileDropdown, StreakWidget, CourseCard, AssignmentItem, Sidebar gibi alt bileşenlere ayırın; useClickOutside gibi tekrarlanabilir mantıkları custom hook olarak çıkarın.
- user/courses/assignments verisini ayrı bir constants.ts veya mock API dosyasına taşıyın; renk/durum gibi magic string'ler için ortak bir constants dosyası oluşturun ve loading/error/empty state'leri ekleyin.
...
**Öneriler:**
- Dropdown menüdeki bağlantılar şimdilik '#' ile bırakılmış; production-ready olması için gerçek yönlendirme işlevi eklenebilir.
- Sidebar'daki 'Tomorrow' etiketi statik metin olarak kalmış; vade tarihine göre dinamik hesaplanması daha işlevsel olurdu.
- `<style jsx>` Next.js'e özgü bir sözdizimidir; saf React projeleri için inline `<style>` etiketi veya bir CSS dosyası kullanmak daha taşınabilir olur.
- Ödev kenar çubuğundaki 'Tomorrow' etiketi sabit bir string; tarihe göre dinamik hesaplanırsa (örn. '2 days left') daha doğru ve semantik olarak daha tutarlı olur.
...
| Performans | 85 | 80 | — | **83** | Ortalama (J1-J2) |
...
**Öneriler:**
- Sabit renk ve ölçü değerlerini (ör. 'bg-indigo-500') anlamlı değişkenlere veya tema tanımlarına taşıyın.
- Kullanılmayan `index` değişkeni ve `getStatusIcon` içindeki tamamlanmış durum gibi kullanılmayan dalları temizleyin.
- Haftalık özet kartlarındaki 4.2, 87%, 12, 3 gibi sihirli sayıları yukarıdaki data dizilerinden türetin veya en azından isimlendirilmiş sabitler olarak tanımlayın.
- StudentDashboard bileşenini StreakWidget, CourseCard, AssignmentItem ve UpcomingDeadlines gibi alt bileşenlere ayırarak tek-sorumluluk ilkesini güçlendirin ve ana dosyayı kısaltın.

### Performans — 83/100

**Öneriler:**
- `getPriorityColor` ve `getStatusIcon` fonksiyonlarını bileşen dışına taşı veya `useCallback` ile memoize et; böylece her render'da yeniden oluşmazlar.
- Badges listesinde `key={index}` yerine `key={badge.name}` gibi stabil bir ID kullan; sıralama değişirse yeniden render'da sorun yaşanmaz.
- getPriorityColor ve getStatusIcon gibi sabit fonksiyonları component dışına taşıyın; her render'da yeniden oluşturulmazlar.
- streakData.badges map'inde key={index} yerine badge.name gibi stabil bir unique değer kullanın (ileride liste değişirse re-render/reorder sorunlarını önler).

### Güvenlik — 98/100

**Öneriler:**
- İleride gerçek bir API entegrasyonu yapılırsa, kullanıcıdan gelen herhangi bir veri JSX'e yerleştirilmeden önce React'in varsayılan XSS korumasına güvenin, eğer HTML render edilecekse DOMPurify ile sanitize edin.
- API token'larını kesinlikle `localStorage`'a koymayın; güvenli httpOnly cookie'ler tercih edin. Environment variable'ları ilgili framework ön ekleriyle (`NEXT_PUBLIC_`, `VITE_`) kullanın.
- Gelecekte kullanıcı kaynaklı veri (ör. kurs adı, eğitmen adı) render edilecekse DOMPurify ile sanitize edilmiş dangerouslySetInnerHTML veya güvenli render katmanı ekleyin.
- API entegrasyonu eklendiğinde token'ları HttpOnly cookie'de saklayın ve hassas verileri console'a yazdırmaktan kaçının; ayrıca fetch çağrılarında HTTPS zorunluluğu ve CORS politikası tanımlayın.

### Sürdürülebilirlik — 39/100

**Öneriler:**
- Büyük bileşeni (StudentDashboard) Navbar, StreakWidget, CourseCard, AssignmentList gibi alt bileşenlere ayırın ve veri katmanını (user, courses, assignments) ayrı bir constants/data dosyasına taşıyın.
- API çağrıları simüle edilecekse loading, error ve empty durumlarını yöneten bir yapı (örneğin useFetch custom hook'u) ekleyin; aksi takdirde bileşenler sadece sunum amaçlı olsa bile bu durumlar için prop tabanlı bir yapı kurun.
- Bileşeni Navbar, CourseCard, AssignmentItem, StreakWidget ve Sidebar gibi alt bileşenlere ayırın; ortak mantık için useClickOutside gibi custom hook'lar çıkarın.
- user, courses, assignments ve streakData verilerini ayrı bir constants/data dosyasına taşıyın; priority/status için enum veya sabit dizileri kullanın ve loading/error/empty state'lerini ekleyin.
...
**Öneriler:**
- Sidebar'daki 'Weekly Overview' bölümü, istenen ödev tarihleri dışında ek bir özellik; prompt buna izin verse de minimal şekilde kalması istenirse opsiyonel tutulabilir.
- Tüm veri hard-coded; eğer production-ready ve dinamik yapı isteniyorsa API entegrasyonu veya state management eklenmesi düşünülebilir.
- Mobil görünümde (md altı) sidebar'ı ana içerik altına alıp 'Upcoming Deadlines' bölümünü açılır/kapanır bir panel olarak da sunabilirsin.
- Kurs kartlarına tıklandığında ilgili derse navigasyon veya detay paneli açılması gibi gerçek bir etkileşim ekleyerek fonksiyonelliği artırabilirsin.
...
| Metrik | J1 | J2 | J3 | Final | Çözüm Yöntemi |
|---|---|---|---|---|---|
| Okunabilirlik | 85 | 72 | — | **79** | Ortalama (J1-J2) |
| Performans | 85 | 82 | — | **84** | Ortalama (J1-J2) |
| Güvenlik | 100 | 80 | — | **90** | Ortalama (J1-J2) |
| Sürdürülebilirlik | 42 | 38 | — | **40** | Ortalama (J1-J2) |
...
**Öneriler:**
- Kodu daha küçük bileşenlere ayırarak okunabilirliği artırın (ör. TopNav, StreakWidget, CoursesGrid, DeadlinesSidebar).
- Kullanılmayan 'BookOpen' ikonunu import listesinden kaldırın.
- Extract the dashboard into smaller sub-components (TopNav, StreakWidget, CourseCard, DeadlinesSidebar, QuickStatsCard) to bring each piece under ~150 lines and improve navigability.
- Remove the unused BookOpen import and replace the non-standard 'indigo-150' class with a valid Tailwind shade (e.g., 'indigo-200') to avoid reader confusion.

### Performans — 84/100

**Öneriler:**
- STREAK_DATA.weeklyProgress map'inde `key` olarak `index` yerine `item.day` gibi benzersiz bir değer kullanın, liste sıralaması değişmese bile iyi pratiktir.
- Bileşenler şu an basit olduğu için memoization'a ihtiyaç yok, ancak ileride alt bileşenlere ayrılırsa `useCallback` ve `React.memo` ile gereksiz render'ları önleyin.
- Statik dekoratif elementlerdeki 'animate-bounce' ve 'animate-pulse' gibi sürekli CSS animasyonlarını kaldırın veya hover/state koşullu hale getirerek gereksiz repaint/reflow'dan kaçının.
- Kurs kartı ve deadline gibi tekrar eden öğeleri React.memo ile sarın; ayrıca uygulama büyüdükçe React.lazy + Suspense ile dashboard bölümlerini kod bölümlemesi (code splitting) yaparak ilk yükleme performansını artırın.

### Güvenlik — 90/100

**Öneriler:**
- Arama kutusu gibi kullanıcı girdisi alanlarına client-side doğrulama (maksimum uzunluk, özel karakter filtrelemesi) eklenerek savunma derinliği artırılabilir.
- Gelecekte API entegrasyonu yapılırsa, sunucudan dönen hata mesajlarının doğrudan kullanıcıya gösterilmesi stack trace sızıntısına yol açabilir; hata mesajları kullanıcıya gösterilmeden önce sanitize edilmelidir.
- Add input sanitization/escape handling for any future content rendered from user sources (e.g., announcements, course descriptions) before they reach the DOM.
- If future iterations include authentication, store tokens in HttpOnly cookies rather than localStorage and avoid exposing user email/PII directly in JSX without proper context.

### Sürdürülebilirlik — 40/100

**Öneriler:**
- Navbar, StreakWidget, CourseCard, DeadlineItem gibi alt bileşenleri ayrı dosyalara çıkararak modülariteyi artır.
- Sabit verileri (COURSES_DATA, DEADLINES_DATA vb.) ayrı bir constants/data dosyasına taşı ve iş mantığını sunumdan ayırmak için özel hook'lar kullan.
- Bileşeni küçük, yeniden kullanılabilir alt bileşenlere ayırın: <TopNavBar>, <ProfileDropdown>, <NotificationDropdown>, <StreakWidget>, <CourseCard>, <DeadlineItem>, <ProgressBar> gibi; ayrıca useClickOutside gibi custom hook'lar çıkarın.
- Urgency seviyeleri ('high'/'medium'/'low') ve renk eşleştirmeleri için constants.ts dosyasında enum/sabit diziler tanımlayın; mock data yapısını bir API servis katmanı ile soyutlayarak loading/error/empty state yönetimini ekleyin.
...
**Öneriler:**
- Dashboard'da belirtilen 3 aktif kurs yerine 3 kurs doğru şekilde gösterilmiş, ancak 'grid of 3 active courses' ifadesine ek olarak kursların yan yana gelmesi mobilde tek sütuna düşüyor; prompt'ta tam grid yapısı belirtilmediği için bu kabul edilebilir, fakat istenirse sabit 3 sütunlu grid düzeni daha net vurgulanabilir.
- Navigation bar'daki profil dropdown'ı sorunsuz çalışıyor; bildirim dropdown'ı eklenmiş, prompt'ta istenmemiş olsa da kullanıcı deneyimini artırır, ancak talimat dışı bir ekleme olduğu için gelecekte prompt'a sadık kalma adına istenmeyen bileşenler eklenmemelidir.
- Sidebarda sadece deadlines yerine, prompt'ta belirtilen 'upcoming assignment deadlines' öğesine daha sıkı odaklanmak için ek Monthly Summary kartı kaldırılabilir veya isteğe bağlı bir yardımcı bileşen olarak konumlandırılabilir.
- Profile dropdown'da gerçek kullanıcı adı, bio gibi profil bilgileri statik mock data olarak daha belirgin şekilde gösterilebilir; mevcut hâlde yalnızca e-posta gösteriliyor.
